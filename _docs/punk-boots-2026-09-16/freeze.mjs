// Freeze the 10 Medium Streetwear Punk Boots choices against current source definitions, the preserved,
// hash-verified mesh report, each choice's own fitting tags and the stated active baseline. Run from the
// repository root after `prepare-punk-boots.py mesh`:
//   node --import tsx _docs/punk-boots-2026-09-16/freeze.mjs
//
...
export const DIR = '_docs/punk-boots-2026-09-16';
...
export const SOURCE_MESH = '/Game/Discovery/Characters/Streetwear/Assets/PunkBoots/SK_Streetwear_PunkBoots_M.SK_Streetwear_PunkBoots_M';
export const MATERIAL_SLOT = 'PunkBoots';
export const ITEM_SLOT = 'feet';
...
export const GLB_SHA256 = '122ca4eaaefc527b56e608e4939af6f9a66ec29705f876b1945f8532bd460537';
...
export const MESH_FACTS = {vertices: 4638, triangles: 7442, uvSets: 2, bones: 16, materialSections: 1};
...
// are drift, and none of them is invented or dropped here.
export const FITTING_TAGS = ['Customization.Shape.PushInsideClothes.push_lower_feet',
...
// (not the picker's full touched total of 569, which also counts legacy, review and body-paint states).
export const BASELINE_ADVERTISED = 522;
export const BASELINE_INDEXED = 523;
export const CATALOG_ITEMS = 2866;
// Hashed in the same order and with the same keys as prepare-punk-boots.py's _active_hashes.
...
export function assertCohort(cohort) {
  ...
  assert.deepEqual(cohort.meshes, [SOURCE_MESH], 'cohort must name exactly the punk-boot skeletal mesh');
...
export function assertMeshReport(report, inventory, shaFn = sha) {
  ...
  assert.equal(report.sha256, GLB_SHA256, 'mesh report GLB is not the preserved punk-boot conversion');
  assert(isSourceMesh(report.source), 'mesh report source is not the punk-boot skeletal mesh');
...
export function sourceSlotsFromDto(dto) {
  ...
  assert.equal(slots.length, 1, 'the punk-boot mesh must export exactly one material slot');
...
export function assertDefinition(item, definition) {
  ...
  // A boot must activate its own row of fitting tags; a missing or empty ActivatesTags is drift here.
...
export function assertResolved(item, definition, resolved, slots) {
  ...
  assert.equal(part.skeletalMesh, SOURCE_MESH, `visible part is not the punk-boot mesh: ${item.id}`);
...
export function resolveChoice(item, definition, slots, resolveSourceOutfit) {
  ...
  // The visible boot must hand this choice's own tags to the production fitting logic (lower foot pushed
  // inside, trousers shrunk by both the low-boot and wide-boot rules).
