// Freeze the 10 Medium Military Sniper Boots choices against current source definitions, the preserved,
// hash-verified mesh report, each choice's own fitting tags and the stated active baseline. Run from the
// repository root after `prepare-sniper-boots.py mesh`:
//   node --import tsx _docs/sniper-boots-2026-09-16/freeze.mjs
//
...
export const DIR = '_docs/sniper-boots-2026-09-16';
...
export const SOURCE_MESH = '/Game/Discovery/Characters/Military/Assets/SniperBoots/SK_Military_SniperBoots_M.SK_Military_SniperBoots_M';
export const MATERIAL_SLOT = 'SniperBoots';
...
export const GLB_SHA256 = '69f401a4311275046e5ddd3ed25e187ad0a912fc983f4a2fb2a3c40f3ada47b3';
...
export const MESH_FACTS = {vertices: 3080, triangles: 5028, uvSets: 2, bones: 12, materialSections: 1};
// Every boot activates exactly these two tags: trousers shrink into the low boots and the lower foot is pushed
// inside. Source order is not significant; missing, empty, repeated or extra tags are drift, and none of them
// is invented or dropped here.
...
// (not the picker's full touched total of 549, which also counts legacy, review and body-paint states).
export const BASELINE_ADVERTISED = 502;
export const BASELINE_INDEXED = 503;
export const CATALOG_ITEMS = 2866;
// Hashed in the same order and with the same keys as prepare-sniper-boots.py's _active_hashes.
...
export function assertCohort(cohort) {
  ...
  assert.deepEqual(cohort.meshes, [SOURCE_MESH], 'cohort must name exactly the sniper-boot skeletal mesh');
...
export function assertMeshReport(report, inventory, shaFn = sha) {
  ...
  assert.equal(report.sha256, GLB_SHA256, 'mesh report GLB is not the preserved sniper-boot conversion');
  assert(isSourceMesh(report.source), 'mesh report source is not the sniper-boot skeletal mesh');
...
export function sourceSlotsFromDto(dto) {
  ...
  assert.equal(slots.length, 1, 'the sniper-boot mesh must export exactly one material slot');
...
export function assertResolved(item, definition, resolved, slots) {
  ...
  assert.equal(part.skeletalMesh, SOURCE_MESH, `visible part is not the sniper-boot mesh: ${item.id}`);
...
export function resolveChoice(item, definition, slots, resolveSourceOutfit) {
  ...
  // The visible boot must hand this choice's own tags to the production fitting logic (trousers tucked into
  // the low boots, lower foot pushed inside).
