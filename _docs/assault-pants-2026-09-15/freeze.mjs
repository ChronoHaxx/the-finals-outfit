// Freeze the 15 Medium Military Assault Pants choices against current source definitions, the preserved,
// hash-verified mesh report and the stated active baseline. Run from the repository root after
// `prepare-assault-pants.py mesh`:
//   node --import tsx _docs/assault-pants-2026-09-15/freeze.mjs
//
...
export const DIR = '_docs/assault-pants-2026-09-15';
...
export const SOURCE_MESH = '/Game/Discovery/Characters/Military/Assets/AssaultPants/SK_Military_AssaultPants_M.SK_Military_AssaultPants_M';
export const MATERIAL_SLOT = 'AssaultPants';
export const ITEM_SLOT = 'lowerBody';
export const CONTEXT = 'Customization.Archetype.Medium';
export const EXPECTED_COUNT = 15;
export const MESH_REPORT = `${DIR}/mesh-report.json`;
// The preserved conversion handed over with this batch.
export const GLB_SHA256 = '99a2c56a4cbbf9628a43083a4c60595a1fddfb4c06cf11c5f01eb9b15e3dd9b6';
// Active index before this batch; indexed counts catalog choices with an advertised assembly or a skin pair.
export const BASELINE_ADVERTISED = 442;
export const BASELINE_INDEXED = 443;
export const CATALOG_ITEMS = 2866;
// Hashed in the same order and with the same keys as prepare-assault-pants.py's _active_hashes.
...
export function assertCohort(cohort) {
  ...
  assert.deepEqual(cohort.meshes, [SOURCE_MESH], 'cohort must name exactly the assault-pants skeletal mesh');
...
export function assertMeshReport(report, inventory, shaFn = sha) {
  ...
  assert.equal(report.sha256, GLB_SHA256, 'mesh report GLB is not the preserved assault-pants conversion');
  assert(isSourceMesh(report.source), 'mesh report source is not the assault-pants skeletal mesh');
...
export function sourceSlotsFromDto(dto) {
  ...
  assert.equal(slots.length, 1, 'the assault-pants mesh must export exactly one material slot');
...
// Morph targets of the exported LOD0, in source order; names must be usable as a lookup dictionary.
...
// Counts as the picker shows them: advertised assemblies, and catalog choices with an assembly or skin pair.
...
export function assertResolved(item, definition, resolved, slots) {
  ...
  assert.equal(part.skeletalMesh, SOURCE_MESH, `visible part is not the assault-pants mesh: ${item.id}`);
...
async function main() {
  ...
  writeFrozenSet([
    [`${DIR}/resolved-cohort.json`, {...cohort, context: CONTEXT, items, meshReport, sourceSlots: slots, lodSummary,
      compatibility: `All ${EXPECTED_COUNT} current source package hashes and decoded properties exactly equal the existing definitions (packages located by unique case-insensitive path; letter case differs only for ${JSON.stringify(caseOnly)}); each resolves one visible Medium part on the preserved, hash-verified assault-pants skeletal mesh with its effective ${MATERIAL_SLOT} material. The converted mesh keeps the source DTO morph targets ${JSON.stringify(morphNames)} by name.`}],
    [`${DIR}/batch.json`, {cohort: `${DIR}/resolved-cohort.json`, ids: items.map(item => item.id)}],
    [`${DIR}/adapter-baseline.json`, {hashes: baselineHashes, counts: baseline}],
  ]);
