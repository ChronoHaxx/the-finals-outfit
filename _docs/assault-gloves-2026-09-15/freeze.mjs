// Freeze the 14 Medium Military Assault Gloves choices against current source definitions, the preserved,
// hash-verified mesh report and the stated active baseline. Run from the repository root after
// `prepare-assault-gloves.py mesh`:
//   node --import tsx _docs/assault-gloves-2026-09-15/freeze.mjs
//
...
export const DIR = '_docs/assault-gloves-2026-09-15';
...
export const SOURCE_MESH = '/Game/Discovery/Characters/Military/Assets/AssaultGloves/SK_Military_AssaultGloves_M.SK_Military_AssaultGloves_M';
export const MATERIAL_SLOT = 'AssaultGloves';
...
export const GLB_SHA256 = '6d493ca9147d0c6ab538362ab1684b00f60f80e15ecaf0acb5db857996c384a6';
...
export const MESH_FACTS = {vertices: 2804, triangles: 4880, uvSets: 2, bones: 46, materialSections: 1};
// Active index before this batch; indexed counts catalog choices with an advertised assembly or a skin pair.
export const BASELINE_ADVERTISED = 456;
export const BASELINE_INDEXED = 457;
export const CATALOG_ITEMS = 2866;
// Hashed in the same order and with the same keys as prepare-assault-gloves.py's _active_hashes.
...
export function assertCohort(cohort) {
  ...
  assert.deepEqual(cohort.meshes, [SOURCE_MESH], 'cohort must name exactly the assault-gloves skeletal mesh');
...
export function assertMeshReport(report, inventory, shaFn = sha) {
  ...
  assert.equal(report.sha256, GLB_SHA256, 'mesh report GLB is not the preserved assault-gloves conversion');
  assert(isSourceMesh(report.source), 'mesh report source is not the assault-gloves skeletal mesh');
...
export function sourceSlotsFromDto(dto) {
  ...
  assert.equal(slots.length, 1, 'the assault-gloves mesh must export exactly one material slot');
...
export function assertDefinition(item, definition) {
  ...
  assert.deepEqual(definition.properties.ActivatesTags ?? [], [], `activated tags are not admitted: ${item.id}`);
}
...
export function assertResolved(item, definition, resolved, slots) {
  ...
  assert.equal(part.skeletalMesh, SOURCE_MESH, `visible part is not the assault-gloves mesh: ${item.id}`);
...
export function resolveChoice(item, definition, slots, resolveSourceOutfit) {
  ...
  assert.deepEqual(outfit.fittingTags, [], `fitting tags are not admitted: ${item.id}`);
