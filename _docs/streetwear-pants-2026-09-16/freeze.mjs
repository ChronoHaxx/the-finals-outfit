// Freeze the 10 Medium Streetwear Pants choices against current source definitions, the preserved,
// hash-verified mesh report, each choice's own fitting tags and the stated active baseline. Run from the
// repository root after `prepare-streetwear-pants.py mesh`:
//   node --import tsx _docs/streetwear-pants-2026-09-16/freeze.mjs
//
...
export const DIR = '_docs/streetwear-pants-2026-09-16';
...
export const SOURCE_MESH = '/Game/Discovery/Characters/Streetwear/Assets/Pants/SK_Streetwear_Pants_M.SK_Streetwear_Pants_M';
export const MATERIAL_SLOT = 'Pants';
export const ITEM_SLOT = 'lowerBody';
...
export const GLB_SHA256 = '06769c4645f87fd5500f0aaafd26dc9136acef1a59e0950b55a2b4031fa93581';
...
// The viewer drives them, never this freeze.
...
export const MESH_FACTS = {vertices: 2172, triangles: 3673, uvSets: 1, bones: 22, materialSections: 1};
...
// extra tags are drift, and none of them is invented or dropped here.
export const FITTING_TAGS = ['Customization.Shape.PushInsideClothes.push_right_calf',
...
export const EXPECTED_ACTIVATED_TAGS_BY_ITEM = Object.fromEntries([
...
].map(id => [id, [...FITTING_TAGS]]));
// Active index before this batch; indexed counts catalog choices with an advertised assembly or skin pair
// (not the picker's full touched total of 559, which also counts legacy, review and body-paint states).
export const BASELINE_ADVERTISED = 512;
export const BASELINE_INDEXED = 513;
export const CATALOG_ITEMS = 2866;
// Hashed in the same order and with the same keys as prepare-streetwear-pants.py's _active_hashes.
...
export function assertCohort(cohort) {
  ...
  assert.deepEqual(cohort.meshes, [SOURCE_MESH], 'cohort must name exactly the streetwear-pants skeletal mesh');
...
export function assertMeshReport(report, inventory, shaFn = sha) {
  ...
  assert.equal(report.sha256, GLB_SHA256, 'mesh report GLB is not the preserved streetwear-pants conversion');
  assert(isSourceMesh(report.source), 'mesh report source is not the streetwear-pants skeletal mesh');
...
export function sourceSlotsFromDto(dto) {
  ...
  assert.equal(slots.length, 1, 'the streetwear-pants mesh must export exactly one material slot');
...
export function assertDefinition(item, definition) {
  ...
  // A pair of pants must activate its own row of fitting tags; a missing or empty ActivatesTags is drift here.
...
export function assertResolved(item, definition, resolved, slots) {
  ...
  assert.equal(part.skeletalMesh, SOURCE_MESH, `visible part is not the streetwear-pants mesh: ${item.id}`);
...
export function resolveChoice(item, definition, slots, resolveSourceOutfit) {
  ...
  // The visible pants must hand this choice's own tags to the production fitting logic (right calf, thighs and
  // upper thighs pushed inside the clothing).
