// Freeze the 17 Medium Starter Set Sneaker choices against current source definitions and the preserved,
// hash-verified mesh report. Run from the repository root after `prepare-starter-sneakers.py mesh`:
//   node --import tsx _docs/starter-sneakers-2026-09-15/freeze.mjs
//
...
export const DIR = '_docs/starter-sneakers-2026-09-15';
// Metadata paths are as in the racing-glove freeze reference.
...
export const SOURCE_MESH = '/Game/Discovery/Characters/StarterSet/Assets/Sneakers/SK_Sneakers_M.SK_Sneakers_M';
export const MATERIAL_SLOT = 'Sneakers';
export const ITEM_SLOT = 'feet';
export const CONTEXT = 'Customization.Archetype.Medium';
export const EXPECTED_COUNT = 17;
export const MESH_REPORT = `${DIR}/mesh-report.json`;
// The preserved conversion handed over with this batch.
export const GLB_SHA256 = 'b7cafddaf56c61c431ff07470297d6bae3303dc981b66f700c71dc367f2513c3';
...
export function assertCohort(cohort) {
  ...
  assert.deepEqual(cohort.meshes, [SOURCE_MESH], 'cohort must name exactly the starter-sneaker skeletal mesh');
...
export function assertMeshReport(report, inventory, shaFn = sha) {
  ...
  assert.equal(report.sha256, GLB_SHA256, 'mesh report GLB is not the preserved starter-sneaker conversion');
  assert(isSourceMesh(report.source), 'mesh report source is not the starter-sneaker skeletal mesh');
...
export function sourceSlotsFromDto(dto) {
  ...
  assert.equal(slots.length, 1, 'the starter-sneaker mesh must export exactly one material slot');
...
export function assertResolved(item, definition, resolved, slots) {
  ...
  assert.equal(part.skeletalMesh, SOURCE_MESH, `visible part is not the starter-sneaker mesh: ${item.id}`);
