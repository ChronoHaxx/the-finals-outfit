// Freeze the 11 Medium Military Combat Gloves choices against current source definitions, the preserved,
// hash-verified mesh report, the full-glove fitting tags and the stated active baseline. Run from the
// repository root after `prepare-combat-gloves.py mesh`:
//   node --import tsx _docs/combat-gloves-2026-09-15/freeze.mjs
//
...
export const DIR = '_docs/combat-gloves-2026-09-15';
...
export const SOURCE_MESH = '/Game/Discovery/Characters/Military/Assets/CombatGloves/SK_Military_CombatGloves_M.SK_Military_CombatGloves_M';
export const MATERIAL_SLOT = 'CombatGloves';
...
export const GLB_SHA256 = 'fb1ec530587e17a0a571e5deb5eed2a30074857f66db2e539e3bc2ce4fa32c84';
// The exported Medium mesh as handed over: original morph targets in source order, and the report summary.
// The two shrink morphs are sleeve fitting; production outfit rules activate them, never this freeze.
export const MORPH_NAMES = ['medium_male', 'medium_female', 'shrink_gloves_under_jacket', 'shrink_gloves_under_jacket_right'];
export const MESH_FACTS = {vertices: 5676, triangles: 9592, uvSets: 2, bones: 53, materialSections: 1};
// Every full-glove definition activates exactly these tags (source order is not significant).
export const FITTING_TAGS = ['Customization.HideMesh.NailsCovered', 'Customization.Shape.PushInsideClothes.push_gloves'];
// Active index before this batch; indexed counts catalog choices with an advertised assembly or skin pair.
export const BASELINE_ADVERTISED = 470;
export const BASELINE_INDEXED = 471;
export const CATALOG_ITEMS = 2866;
// Hashed in the same order and with the same keys as prepare-combat-gloves.py's _active_hashes.
...
export function assertCohort(cohort) {
  ...
  assert.deepEqual(cohort.meshes, [SOURCE_MESH], 'cohort must name exactly the combat-gloves skeletal mesh');
...
export function assertMeshReport(report, inventory, shaFn = sha) {
  ...
  assert.equal(report.sha256, GLB_SHA256, 'mesh report GLB is not the preserved combat-gloves conversion');
  assert(isSourceMesh(report.source), 'mesh report source is not the combat-gloves skeletal mesh');
...
export function sourceSlotsFromDto(dto) {
  ...
  assert.equal(slots.length, 1, 'the combat-gloves mesh must export exactly one material slot');
...
export function assertDefinition(item, definition) {
  ...
  // A full glove must activate both fitting tags; a missing or empty ActivatesTags is drift here.
...
export function assertResolved(item, definition, resolved, slots) {
  ...
  assert.equal(part.skeletalMesh, SOURCE_MESH, `visible part is not the combat-gloves mesh: ${item.id}`);
...
export function resolveChoice(item, definition, slots, resolveSourceOutfit) {
  ...
  // The visible glove must hand both tags to the production fitting logic (nail hiding, push-inside shapes).
