// Freeze the 11 Medium Starter Set T-Shirt choices against current source definitions, the preserved,
// hash-verified mesh report, the four torso fitting tags and the stated active baseline. Run from the
// repository root after `prepare-starter-tshirts.py mesh`:
//   node --import tsx _docs/starter-tshirts-2026-09-15/freeze.mjs
//
...
export const DIR = '_docs/starter-tshirts-2026-09-15';
...
export const SOURCE_MESH = '/Game/Discovery/Characters/StarterSet/Assets/TShirt/SK_StarterSet_TShirt_M.SK_StarterSet_TShirt_M';
export const MATERIAL_SLOT = 'TShirt';
export const ITEM_SLOT = 'upperBody';
export const CONTEXT = 'Customization.Archetype.Medium';
export const EXPECTED_COUNT = 11;
export const MESH_REPORT = `${DIR}/mesh-report.json`;
// The preserved conversion handed over with this batch.
export const GLB_SHA256 = '6aa7672320be90b1692a0e61cb4e75d82c68b5c18139a38ec0f81c7cd4ee5d96';
// The exported Medium mesh as handed over: original morph targets in source order, and the report summary.
// Only the two Medium body morphs; this family has no sleeve morphs. The viewer drives them, never this freeze.
export const MORPH_NAMES = ['medium_male', 'medium_female'];
export const MESH_FACTS = {vertices: 1761, triangles: 2778, uvSets: 2, bones: 27, materialSections: 1};
// Every T-shirt definition activates exactly these tags (source order is not significant); no glove/nail tags.
export const FITTING_TAGS = ['Customization.Shape.PushInsideClothes.push_torso', 'Customization.HideMesh.WearingLongJacket',
...
export const BASELINE_ADVERTISED = 481;
export const BASELINE_INDEXED = 482;
export const CATALOG_ITEMS = 2866;
// Hashed in the same order and with the same keys as prepare-starter-tshirts.py's _active_hashes.
...
export function assertCohort(cohort) {
  ...
  assert.deepEqual(cohort.meshes, [SOURCE_MESH], 'cohort must name exactly the starter-tshirt skeletal mesh');
...
export function assertMeshReport(report, inventory, shaFn = sha) {
  ...
  assert.equal(report.sha256, GLB_SHA256, 'mesh report GLB is not the preserved starter-tshirt conversion');
  assert(isSourceMesh(report.source), 'mesh report source is not the starter-tshirt skeletal mesh');
...
export function sourceSlotsFromDto(dto) {
  ...
  assert.equal(slots.length, 1, 'the starter-tshirt mesh must export exactly one material slot');
...
export function assertDefinition(item, definition) {
  ...
  // A T-shirt must activate all four fitting tags; a missing or empty ActivatesTags is drift here.
...
export function assertResolved(item, definition, resolved, slots) {
  ...
  assert.equal(part.skeletalMesh, SOURCE_MESH, `visible part is not the starter-tshirt mesh: ${item.id}`);
...
export function resolveChoice(item, definition, slots, resolveSourceOutfit) {
  ...
  // The visible T-shirt must hand all four tags to the production fitting logic (torso push, shrink-wraps, jacket hiding).
