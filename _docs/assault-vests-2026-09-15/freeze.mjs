// Freeze the 10 Medium Military Assault Vest choices against current source definitions, the preserved,
// hash-verified mesh report, each choice's own fitting tags and the stated active baseline. Run from the
// repository root after `prepare-assault-vests.py mesh`:
//   node --import tsx _docs/assault-vests-2026-09-15/freeze.mjs
//
...
export const DIR = '_docs/assault-vests-2026-09-15';
...
export const SOURCE_MESH = '/Game/Discovery/Characters/Military/Assets/AssaultVest/SK_Military_AssaultVest_M.SK_Military_AssaultVest_M';
export const MATERIAL_SLOT = 'AssaultVest';
export const ITEM_SLOT = 'upperBody';
...
export const GLB_SHA256 = '2c0e14a13c5c3f1a20ac54b28f8f280afde446bfad0edc43a31931d199c1b0e2';
...
// reordered pair is a different export. The viewer drives them, never this freeze.
export const MORPH_NAMES = ['medium_female', 'medium_male'];
export const MESH_FACTS = {vertices: 19020, triangles: 29023, uvSets: 2, bones: 52, materialSections: 1};
...
export const COMMON_TAGS = ['Customization.HideMesh.WearingLongJacket',
...
export const UNDER_COLLAR = 'Customization.Shape.ShrinkWrap.shrink_under_collar';
...
export const SHRINKS_UNDER_COLLAR = {
  'military-assaultvest-polyester-black': true,
...
export const EXPECTED_ACTIVATED_TAGS_BY_ITEM = Object.fromEntries(Object.entries(SHRINKS_UNDER_COLLAR)
  .map(([id, collar]) => [id, collar ? [...COMMON_TAGS, UNDER_COLLAR] : [...COMMON_TAGS]]));
// Active index before this batch; indexed counts catalog choices with an advertised assembly or skin pair.
export const BASELINE_ADVERTISED = 492;
export const BASELINE_INDEXED = 493;
export const CATALOG_ITEMS = 2866;
// Hashed in the same order and with the same keys as prepare-assault-vests.py's _active_hashes.
...
export const tagsFor = itemId => EXPECTED_ACTIVATED_TAGS_BY_ITEM[itemId];

// Exactly this choice's own fitting tags in any order; missing, empty, repeated, other tags or an
// unknown choice id are drift.
export const isFittingTags = (itemId, tags) => {
  const expected = tagsFor(itemId);
  return !!expected && Array.isArray(tags) && tags.every(tag => typeof tag === 'string')
    && JSON.stringify([...tags].sort()) === JSON.stringify([...expected].sort());
};
...
export function assertCohort(cohort) {
  ...
  assert.deepEqual([...ids].sort(), Object.keys(EXPECTED_ACTIVATED_TAGS_BY_ITEM).sort(),
    'cohort choices differ from the per-choice fitting tag mapping');
  assert.deepEqual(cohort.meshes, [SOURCE_MESH], 'cohort must name exactly the assault-vest skeletal mesh');
...
export function assertMeshReport(report, inventory, shaFn = sha) {
  ...
  assert.equal(report.sha256, GLB_SHA256, 'mesh report GLB is not the preserved assault-vest conversion');
  assert(isSourceMesh(report.source), 'mesh report source is not the assault-vest skeletal mesh');
...
export function sourceSlotsFromDto(dto) {
  ...
  assert.equal(slots.length, 1, 'the assault-vest mesh must export exactly one material slot');
...
export function assertDefinition(item, definition) {
  ...
  // A vest must activate its own row of fitting tags; a missing or empty ActivatesTags is drift here, and
  // the under-collar shrink wrap belongs only to the choices that already carry it.
  assert(isFittingTags(item.id, definition.properties.ActivatesTags),
    `activated tags ${JSON.stringify(definition.properties.ActivatesTags)} are not exactly ${JSON.stringify(tagsFor(item.id))}: ${item.id}`);
}
...
export function assertResolved(item, definition, resolved, slots) {
  ...
  assert.equal(part.skeletalMesh, SOURCE_MESH, `visible part is not the assault-vest mesh: ${item.id}`);
...
export function resolveChoice(item, definition, slots, resolveSourceOutfit) {
  ...
  // The visible vest must hand this choice's own tags to the production fitting logic (jacket push and
  // hiding, wrists, gloves, skirts, badge, upper-back item, and the collar only where it belongs).
  assert(isFittingTags(item.id, outfit.fittingTags),
    `resolved fitting tags ${JSON.stringify(outfit.fittingTags)} are not exactly ${JSON.stringify(tagsFor(item.id))}: ${item.id}`);
...
async function main() {
  ...
  const underCollar = items.filter(item => item.fittingTags.includes(UNDER_COLLAR)).map(item => item.id);
  writeFrozenSet([
    [FROZEN_OUTPUTS.cohort, {...cohort, context: CONTEXT, items, meshReport, sourceSlots: slots, lodSummary,
      activatedTagsByItem: EXPECTED_ACTIVATED_TAGS_BY_ITEM,
      compatibility: `All ${EXPECTED_COUNT} current source package hashes and decoded properties exactly equal the existing definitions (packages located by unique case-insensitive path; letter case differs only for ${JSON.stringify(caseOnly)}); each resolves one visible Medium part on the preserved, hash-verified assault-vest skeletal mesh with its effective ${MATERIAL_SLOT} material and activates exactly its own fitting tags: the shared ${JSON.stringify(COMMON_TAGS)}, plus ${JSON.stringify(UNDER_COLLAR)} for ${underCollar.length} of the ${EXPECTED_COUNT} choices ${JSON.stringify(underCollar)} and for no other. The converted mesh keeps the source DTO morph targets ${JSON.stringify(morphNames)} by name, with no default weights.`}],
  ...
  console.log(JSON.stringify({items: items.length, slots, morphNames, baseline,
    ...
    underCollar, identities: [...new Set(items.map(item => item.identity.matches ?? 1))]}));
}
