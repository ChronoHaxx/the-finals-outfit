"""Focused negative contracts for the Starter Set T-Shirt adapter and freeze, without game assets.
...
source-only cohort.json is the only fixture; values under test come from the modules, not copies.
  ...
  C:/ProgramData/anaconda3/python.exe -B tests/test_starter_tshirts_adapter.py -v
"""
import hashlib
...
ADAPTER = ROOT / 'scripts' / 'shader-probe' / 'prepare-starter-tshirts.py'
FREEZE = ROOT / '_docs' / 'starter-tshirts-2026-09-15' / 'freeze.mjs'
RESOLVER = ROOT / 'src' / 'rig' / 'SourceAssembly.ts'
COHORT = ROOT / '_docs' / 'starter-tshirts-2026-09-15' / 'cohort.json'
DEFAULT_MATERIAL = '/Game/Discovery/Characters/StarterSet/Assets/TShirt/M_StarterSet_TShirt.M_StarterSet_TShirt'
# The previous (gloves) family's morphs and tags: stale values that must never pass for this family.
GLOVE_SLEEVE_MORPHS = ['shrink_gloves_under_jacket', 'shrink_gloves_under_jacket_right']
GLOVE_TAGS = ['Customization.HideMesh.NailsCovered', 'Customization.Shape.PushInsideClothes.push_gloves']
...
def load_adapter():
    ...
    spec = real('prepare_starter_tshirts', ADAPTER)
...
def mesh_document(names, slot, uv_sets=2, bones=27):
...
class AdapterGuards(unittest.TestCase):
    def setUp(self):
        ...
        self.other_mesh = self.a.SOURCE_MESH.replace('TShirt_M', 'TShirt_F')
    ...
    def stage_mesh(self, slot=None, morphs=None, glb_names=None, dto_source=None, record_path=None,
        ...
        dto = folder / 'SK_StarterSet_TShirt_M.mesh.json'
        ...
        path = a.WORK / 'meshes' / 'SK_StarterSet_TShirt_M.glb'
        path.parent.mkdir(parents=True, exist_ok=True)
    ...
    def test_report_must_name_the_preserved_glb(self):
        ...
        with self.assertRaisesRegex(SystemExit, 'preserved starter-tshirt conversion'):
    ...
    def test_mesh_contract_rejects_other_mesh_slot_source_and_lost_or_misnamed_morphs(self):
        ...
        self.assertEqual((recorded['morphNames'], recorded['slots'], recorded['bones']), (a.MORPH_NAMES, slots, 27))
        ...
        for change, reason in (({'slot': 'CombatGloves'}, 'TShirt material slot'),
                               ...
                               ({'facts': {'vertices': 1760}}, 'differs from the exported mesh'),
    ...
    def test_glb_keeps_original_uv_sets_skinning_and_inactive_named_morphs(self):
        ...
        self.assertEqual(a._require_glb_geometry({**report, 'morphs': list(names), 'bones': list(range(27))}, dto), names)
        ...
        # A body morph switched on by default (mesh or node weights) would reshape every T-shirt.
    ...
    def test_frozen_cohort_keeps_one_tshirt_part_and_exactly_the_fitting_tags(self):
        ...
        part, wrong_slot = rows[0]['effectiveParts'][0], {'slot': 'CombatGloves', 'material': rows[0]['materials'][0]}
        drift = 'does not activate exactly'
        for first, fields, reason in ((tagged({}), {}, drift),  # missing is not valid for a T-shirt
                                      (tagged({'ActivatesTags': []}), {}, drift),
                                      (tagged({'ActivatesTags': tags[:3]}), {}, drift),
                                      (tagged({'ActivatesTags': [*tags[:3], tags[0]]}), {}, drift),  # repeated, one lost
                                      (tagged({'ActivatesTags': [*tags, tags[0]]}), {}, drift),      # repeated
                                      (tagged({'ActivatesTags': [*tags, GLOVE_TAGS[0]]}), {}, drift),
                                      (tagged({'ActivatesTags': GLOVE_TAGS}), {}, drift),
                                      (tagged({'ActivatesTags': [*tags[:3], tags[3].replace('_in', '_out')]}), {}, drift),
                                      ({'fittingTags': []}, {}, drift),
                                      ({'fittingTags': tags[1:]}, {}, drift),
                                      ({'effectiveParts': [{**part, 'mesh': self.other_mesh}]}, {}, 'starter-tshirt mesh'),
                                      ({'effectiveParts': [{**part, 'slots': [wrong_slot]}]}, {}, 'starter-tshirt mesh'),
                                      ({'effectiveParts': [part, {**part, 'sourceIndex': 1}]}, {}, 'starter-tshirt mesh'),
                                      ({'slot': 'hands'}, {}, 'differs from cohort.json'),
...
assert.throws(() => m.assertMeshReport({...report, sha256: 'ff'}, inventory, shaOf('ff')), /preserved starter-tshirt conversion/);
assert.throws(() => m.assertMeshReport(report, inventory, shaOf('ff')), /GLB hash does not match/);
assert.throws(() => m.assertMeshReport({...report, source: m.SOURCE_MESH.replace('TShirt_M', 'TShirt_F')}, inventory, shaOf(m.GLB_SHA256)), /skeletal mesh/);
assert.throws(() => m.assertMeshReport({...report, source: m.SOURCE_MESH.replaceAll('TShirt', 'Hoodie')}, inventory, shaOf(m.GLB_SHA256)), /skeletal mesh/);
assert.throws(() => m.assertMeshReport(report, new Set(), shaOf(m.GLB_SHA256)), /inventory/);
for (const drift of [{bones: 53}, {uvSets: 1}, {vertices: '1761'}, {triangles: 2779}, {materialSections: 2}])
...
// Lost, reordered, renamed, or the gloves family's sleeve morphs added: none is this mesh.
for (const list of [names.slice(0, 1), [], [...names].reverse(), ['medium_male', 'Medium_Female'],
...
const latest = 'Discovery/Content/Discovery/Characters/StarterSet/Assets/TShirt/Skins/Cotton_Black/MI_TShirt_Cotton_Black.uasset';
const saved = latest.replace('Cotton_Black/', 'Cotton_BLACK/');
const other = latest.replace('Cotton_Black/MI_TShirt_Cotton_Black', 'Cotton_Yellow/MI_TShirt_Cotton_Yellow');
const archive = m.foldedIndex([latest, other]), inventoryIndex = m.foldedIndex(new Set([latest, other]));
const tags = m.FITTING_TAGS;
const definition = {formatVersion: 1, id: 'x', source: saved, sourceSha256: 'ab', properties: {Slots: ['upperBody'], ActivatesTags: tags}};
const current = {status: 'ok', package: {path: latest, sha256: 'ab'},
  exports: [{type: 'CharacterCustomizationItem', properties: {Slots: ['upperBody'], ActivatesTags: tags}}]};

// All four T-shirt tags in any order; missing, empty, partial, repeated, extra, renamed or glove tags are drift.
assert.deepEqual(tags, ['Customization.Shape.PushInsideClothes.push_torso', 'Customization.HideMesh.WearingLongJacket',
  'Customization.Shape.ShrinkWrap.shrink_skirt_shorts_in', 'Customization.Shape.ShrinkWrap.shrink_half_skirt_in']);
const gloveTags = ['Customization.HideMesh.NailsCovered', 'Customization.Shape.PushInsideClothes.push_gloves'];
...
for (const drift of [undefined, [], tags.slice(0, 3), [...tags.slice(0, 3), tags[0]], [...tags, tags[0]], [...tags, gloveTags[0]],
                     gloveTags, [...tags.slice(0, 3), 'Customization.Shape.ShrinkWrap.shrink_half_skirt_out'],
                     tags.map(tag => tag.toLowerCase())])
  assert.throws(() => m.assertDefinition({id: 'x'}, {...definition, properties: {Slots: ['upperBody'], ActivatesTags: drift}}), /activated tags/);
...
assert.throws(() => m.assertCurrentDefinition(definition, definition,
  {...current, exports: [{type: 'CharacterCustomizationItem', properties: {Slots: ['hands'], ActivatesTags: tags}}]}), /source properties changed/);
assert.throws(() => m.assertCurrentDefinition(definition, definition,
  {...current, exports: [{type: 'CharacterCustomizationItem', properties: {Slots: ['upperBody'], ActivatesTags: tags.slice(1)}}]}), /source properties changed/);
// Missing, ambiguous (case-only collision, even with one exact spelling; duplicated key) and disagreeing lookups fail.
assert.throws(() => m.resolveDefinitionPackage({...definition, source: saved.replace('MI_TShirt_Cotton_Black', 'MI_Absent')}, archive, inventoryIndex, 'x'), /absent/);
...
assert.throws(() => m.assertResolved(item, definition, {parts: [{...part, skeletalMesh: m.SOURCE_MESH.replace(/_M\b/g, '_F')}]}, slots), /starter-tshirt mesh/);
...
const dir = fs.mkdtempSync(path.join(os.tmpdir(), '.starter-tshirts-freeze-'));
try {
...
const slots = [{slot: m.MATERIAL_SLOT, material: '/Game/Discovery/Characters/StarterSet/Assets/TShirt/M_Default.M_Default'}];
...
const LIGHT = m.SOURCE_MESH.replaceAll('TShirt_M', 'TShirt_L'), HEAVY = m.SOURCE_MESH.replaceAll('TShirt_M', 'TShirt_H');
...
const definition = {formatVersion: 1, id: item.id, source: 'Discovery/Content/TShirt.uasset', sourceSha256: 'ab',
  properties: {Slots: ['UpperBody'], ActivatesTags: [...m.FITTING_TAGS].reverse(),
...
// Medium resolves one visible part on the T-shirt mesh with the cohort material and hands all four fitting tags
...
// Missing, empty, partial or extra (gloves) activated tags are drift for a T-shirt.
for (const tags of [undefined, [], m.FITTING_TAGS.slice(0, 3), [...m.FITTING_TAGS, 'Customization.HideMesh.NailsCovered']])
...
rejects(variant({}, {TagOverrides: [...archetypes, rule(['Customization.HideMesh.WearingLongJacket'], {bOverrideMesh: true})]}), /hidden/);
rejects(variant({}, {TagOverrides: [...archetypes, rule(['Customization.Shape.ShrinkWrap'], {bOverrideMesh: true, ReplacementSkeletalMesh: soft(LIGHT)})]}), /starter-tshirt mesh/);

const medium = fields => ({TagOverrides: [...archetypes, rule([m.CONTEXT], fields)]});
rejects(variant({}, medium({bOverrideMesh: true, ReplacementSkeletalMesh: soft(LIGHT)})), /starter-tshirt mesh/);
...
class FreezeContracts(unittest.TestCase):
    ...
    def test_adapter_and_freeze_share_the_handed_over_contract(self):
        ...
        # The source-index count, not the product UI's full touched total of 528.
        self.assertEqual((adapter.BASELINE_ADVERTISED, adapter.BASELINE_STRUCTURAL, adapter.BASELINE_INDEXED,
                          adapter.CATALOG_ITEMS), (481, 482, 482, 2866))
        self.assertEqual((adapter.MESH_FACTS, adapter.GLB_SHA256[:12]),
                         ({'vertices': 1761, 'triangles': 2778, 'uvSets': 2, 'bones': 27, 'materialSections': 1}, '6aa7672320be'))
    ...
    def test_production_resolver_on_medium_admits_one_tshirt_part_with_its_fitting_tags(self):
