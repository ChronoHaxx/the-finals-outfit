"""Focused negative contracts for the Municipal Gloves adapter and freeze, without game assets.
...
source-only cohort.json is the only fixture; values under test come from the modules, not copies.
  ...
  C:/ProgramData/anaconda3/python.exe -B tests/test_municipal_gloves_adapter.py -v
"""
import hashlib
...
ADAPTER = ROOT / 'scripts' / 'shader-probe' / 'prepare-municipal-gloves.py'
FREEZE = ROOT / '_docs' / 'municipal-gloves-2026-09-16' / 'freeze.mjs'
RESOLVER = ROOT / 'src' / 'rig' / 'SourceAssembly.ts'
COHORT = ROOT / '_docs' / 'municipal-gloves-2026-09-16' / 'cohort.json'
DEFAULT_MATERIAL = '/Game/Discovery/Characters/Municipal/Assets/Gloves/M_Municipal_Gloves.M_Municipal_Gloves'
# The previous glove family, whose four-morph mesh and two-tag contract must not be inherited.
COMBAT_MESH = '/Game/Discovery/Characters/Military/Assets/CombatGloves/SK_Military_CombatGloves_M.SK_Military_CombatGloves_M'
COMBAT_MORPHS = ['medium_male', 'medium_female', 'shrink_gloves_under_jacket', 'shrink_gloves_under_jacket_right']
...
def load_adapter():
    ...
    configure('_docs/combat-gloves-2026-09-15/batch.json', 'combat', 'combat-runtime', 'combat-preview')
    shared = types.SimpleNamespace(d=d, DOCS=Path('combat'), WORK=Path('combat'), RUNTIME=Path('combat'),
                                   PREVIEW=Path('combat'), MESH_REPORT=Path('combat'), progress=None,
    ...
    spec = real('prepare_municipal_gloves', ADAPTER)
...
def mesh_document(names, slot, uv_sets=2):
    ...
    return {'materials': [{'extras': {'sourceSlot': {'MaterialSlotName': slot}, 'sourceMaterial': DEFAULT_MATERIAL}}],
            ...
            'nodes': [{'mesh': 0, 'skin': 0}],
            'skins': [{'joints': list(range(55)), 'inverseBindMatrices': 7}]}
...
class AdapterGuards(unittest.TestCase):
    def setUp(self):
        ...
        self.other_mesh = self.a.SOURCE_MESH.replace('Gloves_M', 'Gloves_F')
    ...
    def stage_mesh(self, slot=None, morphs=None, glb_names=None, dto_source=None, record_path=None,
        ...
        dto = folder / 'SK_Municipal_Gloves_M.mesh.json'
        ...
        path = a.WORK / 'meshes' / 'SK_Municipal_Gloves_M.glb'
        path.parent.mkdir(parents=True, exist_ok=True)
    ...
    def test_every_shared_global_and_hook_is_repointed(self):
        ...
        for name, stale in (('REUSE_TEXTURES', [Path('scripts/generated/shader-probe/accessory-source-v1/opus-textures-01')]),
                            ...
                            ('MARKER', 'shader-probe/prepare-combat-gloves'), ('plan', object())):
            original = getattr(a.d, name)
            setattr(a.d, name, stale)
            with self.assertRaises(SystemExit):
                a._require_configuration()
            setattr(a.d, name, original)
        for name, stale in (('MESH_REPORT', Path('_docs/combat-gloves-2026-09-15/mesh-report.json')), ('progress', print)):
    ...
    def test_supplied_cohort_is_accepted_and_drift_rejected(self):
        ...
        for broken in ({**cohort, 'count': a.EXPECTED_COUNT + 1}, {**cohort, 'items': rest},
                       ...
                       {**cohort, 'items': [{**first, 'slot': 'lowerBody'}, *rest]},
    ...
    def test_report_must_name_the_preserved_glb(self):
        ...
        with self.assertRaisesRegex(SystemExit, 'preserved municipal-gloves conversion'):
    ...
    def test_mesh_contract_rejects_other_mesh_slot_source_and_lost_or_misnamed_morphs(self):
        ...
        self.assertEqual((recorded['morphNames'], recorded['slots'], recorded['bones']), (a.MORPH_NAMES, slots, 55))
        ...
        for change, reason in (({'slot': 'CombatGloves'}, 'Gloves material slot'),
                               ...
                               ({'report_source': COMBAT_MESH}, 'mesh source identity'),
                               ...
                               ({'morphs': [female, male]}, 'not the original'),
                               ({'morphs': COMBAT_MORPHS}, 'not the original'),  # the reference's sleeve-shrink morphs
                               ({'morphs': [male, female, COMBAT_MORPHS[2]]}, 'not the original'),
                               ({'morphs': [male, 'Medium_Female']}, 'not the original'),
                               ({'morphs': [male, 'medium_femal']}, 'not the original'),
                               # Lost, misnamed or invented only by the conversion.
                               ({'glb_names': [male]}, 'GLB morph target names'),
                               ({'glb_names': [male, 'medium_female_1']}, 'GLB morph target names'),
                               ({'glb_names': [female, male]}, 'GLB morph target names'),
                               ({'glb_names': []}, 'GLB morph target names'),
                               ({'glb_names': COMBAT_MORPHS}, 'GLB morph target names'),
                               ({'facts': {'bones': 53}}, 'differs from the exported mesh'),
                               ({'facts': {'vertices': 3103}}, 'differs from the exported mesh'),
                               ({'facts': {'triangles': 9592}}, 'differs from the exported mesh'),
    ...
    def test_glb_keeps_original_uv_sets_skinning_and_inactive_named_morphs(self):
        ...
        self.assertEqual(a._require_glb_geometry({**report, 'morphs': list(names), 'bones': list(range(55))}, dto), names)
        ...
        for broken_mesh in ({**mesh, 'extras': {}},                                          # names lost
                            ...
                            {**mesh, 'extras': {'targetNames': COMBAT_MORPHS}},              # invented
        ...
        # A body-type morph switched on by default (mesh or node weights) would reshape every glove.
    ...
    def test_frozen_cohort_keeps_one_gloves_part_and_exactly_the_three_fitting_tags(self):
        a, cohort, tags = self.a, self.cohort, self.a.FITTING_TAGS
        ...
        def stage(first=None, **fields):
            a.d.write(a.DOCS / 'resolved-cohort.json', {**frozen, **fields, 'items': [{**rows[0], **(first or {})}, *rows[1:]]})

        def tagged(properties):
            return {'definition': {**rows[0]['definition'], 'properties': properties}}
        ...
        stage({'fittingTags': tags[::-1]}); a._require_frozen_cohort(report, slots)
        part, combat = rows[0]['effectiveParts'][0], {'slot': 'CombatGloves', 'material': rows[0]['materials'][0]}
        for first, fields, reason in ((tagged({}), {}, 'does not activate exactly'),  # missing is not valid for full gloves
                                      (tagged({'ActivatesTags': []}), {}, 'does not activate exactly'),
                                      (tagged({'ActivatesTags': tags[:1]}), {}, 'does not activate exactly'),
                                      # The Combat Gloves pair keeps wrist cosmetics visible; these gloves must hide them.
                                      (tagged({'ActivatesTags': [tags[0], tags[2]]}), {}, 'does not activate exactly'),
                                      (tagged({'ActivatesTags': [tags[0], tags[0], tags[2]]}), {}, 'does not activate exactly'),
                                      (tagged({'ActivatesTags': [*tags, tags[1]]}), {}, 'does not activate exactly'),
                                      (tagged({'ActivatesTags': [*tags, 'Customization.Fitting.Gloves']}), {}, 'does not activate exactly'),
                                      (tagged({'ActivatesTags': [tags[0], tags[1], tags[2] + '_right']}), {}, 'does not activate exactly'),
                                      ({'fittingTags': []}, {}, 'does not activate exactly'),
                                      ({'fittingTags': tags[1:]}, {}, 'does not activate exactly'),
                                      ({'fittingTags': [tags[0], tags[2]]}, {}, 'does not activate exactly'),
                                      ({'effectiveParts': [{**part, 'mesh': self.other_mesh}]}, {}, 'municipal-gloves mesh'),
                                      ({'effectiveParts': [{**part, 'mesh': COMBAT_MESH}]}, {}, 'municipal-gloves mesh'),
                                      ({'effectiveParts': [{**part, 'slots': [combat]}]}, {}, 'municipal-gloves mesh'),
                                      ({'effectiveParts': [part, {**part, 'sourceIndex': 1}]}, {}, 'municipal-gloves mesh'),
                                      ({'slot': 'lowerBody'}, {}, 'differs from cohort.json'),
                                      ({}, {'context': 'Customization.Archetype.Light'}, 'not resolved for'),
                                      ({}, {'meshes': [self.other_mesh]}, 'different mesh report'),
                                      ({}, {'sourceSlots': [{**slots[0], 'slot': 'CombatGloves'}]}, 'different mesh report')):
            stage(first, **fields)
            with self.assertRaisesRegex(SystemExit, reason, msg=(first, fields)):
    ...
    def test_preview_keeps_the_baseline_progress_and_its_unadvertised_entry(self):
        ...
        for broken in ({'supported': advertised + implemented + ['knight-pants']},
                       ...
                       {'preview': {**preview, 'marker': 'shader-probe/prepare-combat-gloves'}},
                       {'preview': {**preview, 'implemented': ['military-combatgloves-x']}}):
            stage(**broken)
            with self.assertRaises(SystemExit):
                a._require_preview()
...
// The supplied cohort is accepted; count, slot, mesh, attachment and material-list drift fail.
...
assert.throws(() => m.assertCohort({...cohort, items: [{...cohort.items[0], slot: 'lowerBody'}, ...rest]}), /slot/);
...
assert.throws(() => m.assertMeshReport({...report, sha256: 'ff'}, inventory, shaOf('ff')), /preserved municipal-gloves conversion/);
assert.throws(() => m.assertMeshReport(report, inventory, shaOf('ff')), /GLB hash does not match/);
assert.throws(() => m.assertMeshReport({...report, source: m.SOURCE_MESH.replace('Gloves_M', 'Gloves_F')}, inventory, shaOf(m.GLB_SHA256)), /skeletal mesh/);
assert.throws(() => m.assertMeshReport({...report, source: m.SOURCE_MESH.replaceAll('Municipal', 'Military')}, inventory, shaOf(m.GLB_SHA256)), /skeletal mesh/);
assert.throws(() => m.assertMeshReport(report, new Set(), shaOf(m.GLB_SHA256)), /inventory/);
for (const drift of [{bones: 53}, {uvSets: 1}, {vertices: '3104'}, {triangles: 5417}, {materialSections: 2}])
...
assert.throws(() => m.sourceSlotsFromDto({sourceMaterials: [{MaterialSlotName: 'CombatGloves'}], materials: [{path: '/Game/M.M'}]}), /material slot must be/);
assert.throws(() => m.sourceSlotsFromDto({sourceMaterials: [{MaterialSlotName: slot}, {MaterialSlotName: 'Strap'}],
  materials: [{path: '/Game/M.M'}, {path: '/Game/B.B'}]}), /exactly one/);

// Morph targets are the two original Medium morphs by name, order and count, inactive by default; no sleeve shrink.
const names = m.MORPH_NAMES;
assert.deepEqual(names, ['medium_male', 'medium_female']);
const combatMorphs = [...names, 'shrink_gloves_under_jacket', 'shrink_gloves_under_jacket_right'];
...
for (const list of [names.slice(0, 1), [], [...names].reverse(), combatMorphs, [...names, combatMorphs[2]],
                    [names[0], 'Medium_Female'], [...names, 'medium_extra']])
  assert.throws(() => m.morphNamesFromDto(dtoFor(list)), /original Medium morphs/);
assert.throws(() => m.assertGlbMorphs(glbDoc, names, {...report, morphs: 4}), /morph count/);
assert.throws(() => m.assertGlbMorphs(glbDoc, names, {...report, morphs: combatMorphs}), /report morphs differ/);
assert.throws(() => m.assertGlbMorphs({meshes: [{primitives: [primitive]}]}, names, report), /target names/);
assert.throws(() => m.assertGlbMorphs({meshes: [{extras: {targetNames: [names[0], 'renamed']}, primitives: [primitive]}]}, names, report), /target names/);
assert.throws(() => m.assertGlbMorphs({meshes: [{extras: {targetNames: combatMorphs}, primitives: [primitive]}]}, names, report), /target names/);
...
const latest = 'Discovery/Content/Discovery/Characters/Municipal/Assets/Gloves/Skins/Folder_Case/MI_Gloves.uasset';
const saved = latest.replace('Folder_Case', 'Folder_CASE');
const other = latest.replace('Folder_Case/MI_Gloves', 'Other/MI_Other');
const archive = m.foldedIndex([latest, other]), inventoryIndex = m.foldedIndex(new Set([latest, other]));
const tags = m.FITTING_TAGS;
const definition = {formatVersion: 1, id: 'x', source: saved, sourceSha256: 'ab', properties: {Slots: ['hands'], ActivatesTags: tags}};
const current = {status: 'ok', package: {path: latest, sha256: 'ab'},
  exports: [{type: 'CharacterCustomizationItem', properties: {Slots: ['hands'], ActivatesTags: tags}}]};

// All three full-glove tags in any order; missing (including the Combat Gloves pair without WristsCovered), empty,
// partial, duplicated, extra or renamed tags are drift.
assert.deepEqual(tags, ['Customization.HideMesh.NailsCovered', 'Customization.HideMesh.WristsCovered',
  'Customization.Shape.PushInsideClothes.push_gloves']);
m.assertDefinition({id: 'x'}, definition);
m.assertDefinition({id: 'x'}, {...definition, properties: {...definition.properties, ActivatesTags: [tags[1], tags[2], tags[0]]}});
for (const drift of [undefined, [], tags.slice(0, 1), [tags[0], tags[2]], [tags[0], tags[0], tags[2]], [...tags, tags[1]],
                     [...tags, 'Customization.Fitting.Gloves'], [tags[0], tags[1], tags[2] + '_right'], tags.map(tag => tag.toLowerCase())])
  assert.throws(() => m.assertDefinition({id: 'x'}, {...definition, properties: {Slots: ['hands'], ActivatesTags: drift}}), /activated tags/);
...
assert.throws(() => m.assertCurrentDefinition(definition, definition,
  {...current, exports: [{type: 'CharacterCustomizationItem', properties: {Slots: ['feet'], ActivatesTags: tags}}]}), /source properties changed/);
assert.throws(() => m.assertCurrentDefinition(definition, definition,
  {...current, exports: [{type: 'CharacterCustomizationItem', properties: {Slots: ['hands'], ActivatesTags: tags.slice(1)}}]}), /source properties changed/);
// Missing, ambiguous (case-only collision, even with one exact spelling; duplicated key) and disagreeing lookups fail.
assert.throws(() => m.resolveDefinitionPackage({...definition, source: saved.replace('MI_Gloves', 'MI_Absent')}, archive, inventoryIndex, 'x'), /absent/);
...
assert.throws(() => m.assertResolved(item, definition, {parts: [{...part, skeletalMesh: m.SOURCE_MESH.replace(/_M\b/g, '_F')}]}, slots), /municipal-gloves mesh/);
...
const dir = fs.mkdtempSync(path.join(os.tmpdir(), '.municipal-gloves-freeze-'));
try {
...
const slots = [{slot: m.MATERIAL_SLOT, material: '/Game/Discovery/Characters/Municipal/Assets/Gloves/M_Default.M_Default'}];
...
const LIGHT = m.SOURCE_MESH.replaceAll('Gloves_M', 'Gloves_L'), HEAVY = m.SOURCE_MESH.replaceAll('Gloves_M', 'Gloves_H');
...
const definition = {formatVersion: 1, id: item.id, source: 'Discovery/Content/Gloves.uasset', sourceSha256: 'ab',
  properties: {Slots: ['Hands'], ActivatesTags: [...m.FITTING_TAGS].reverse(),
...
// Medium resolves one visible part on the gloves mesh with the cohort material and hands all three fitting tags
// to the production fitting logic; Light/Heavy overrides stay outside.
const ok = resolve(definition);
assert.deepEqual(ok.effectiveSlots, [{slot: m.MATERIAL_SLOT, material}]);
assert.deepEqual([ok.visible.length, ok.visible[0].skeletalMesh, ok.outfit.fittingTags], [1, m.SOURCE_MESH, [...m.FITTING_TAGS].sort()]);
assert.equal(resolveSourceOutfit([definition], ['Customization.Archetype.Light']).items[item.id].parts[0].skeletalMesh, LIGHT);

// Missing (including WristsCovered alone), empty, partial, duplicated or extra activated tags are drift for a full glove.
const [nails, wrists, push] = m.FITTING_TAGS;
for (const tags of [undefined, [], [nails], [nails, push], [nails, nails, push], [...m.FITTING_TAGS, wrists],
                    [...m.FITTING_TAGS, 'Customization.Fitting.Gloves']])
...
rejects(variant({}, {TagOverrides: [...archetypes, rule(['Customization.HideMesh'], {bOverrideMesh: true})]}), /hidden/);
rejects(variant({}, {TagOverrides: [...archetypes, rule([wrists], {bOverrideMesh: true})]}), /hidden/);
rejects(variant({}, {TagOverrides: [...archetypes, rule(['Customization.Shape.PushInsideClothes'], {bOverrideMesh: true, ReplacementSkeletalMesh: soft(LIGHT)})]}), /municipal-gloves mesh/);

const medium = fields => ({TagOverrides: [...archetypes, rule([m.CONTEXT], fields)]});
rejects(variant({}, medium({bOverrideMesh: true, ReplacementSkeletalMesh: soft(LIGHT)})), /municipal-gloves mesh/);
rejects(variant({}, medium({bOverrideMesh: true})), /hidden/);
rejects(variant({}, medium({bOverrideMesh: true, ReplacementSkeletalMesh: soft(m.SOURCE_MESH.replaceAll('Municipal', 'Military'))})), /municipal-gloves mesh/);
...
rejects(variant({MaterialOverrides: [{Key: 'CombatGloves', Value: soft(material)}]}), /absent slot/);
...
class FreezeContracts(unittest.TestCase):
    ...
    def test_adapter_and_freeze_share_the_handed_over_contract(self):
        ...
        names = ('SOURCE_MESH', 'MATERIAL_SLOT', 'ITEM_SLOT', 'CONTEXT', 'EXPECTED_COUNT', 'GLB_SHA256', 'MORPH_NAMES',
                 'MESH_FACTS', 'FITTING_TAGS', 'BASELINE_ADVERTISED', 'BASELINE_INDEXED', 'CATALOG_ITEMS')
        ...
        # The source-index count, not the product UI's full touched total of 579.
        self.assertEqual((adapter.BASELINE_ADVERTISED, adapter.BASELINE_STRUCTURAL, adapter.BASELINE_INDEXED,
                          adapter.CATALOG_ITEMS), (532, 533, 533, 2866))
    ...
    def test_production_resolver_on_medium_admits_one_gloves_part_with_its_fitting_tags(self):
