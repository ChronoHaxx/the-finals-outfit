"""Focused negative contracts for the Streetwear Punk Boots adapter and freeze, without game assets.
...
source-only cohort.json is the only fixture; values under test come from the modules, not copies.
  ...
  C:/ProgramData/anaconda3/python.exe -B tests/test_punk_boots_adapter.py -v
"""
import hashlib
...
ADAPTER = ROOT / 'scripts' / 'shader-probe' / 'prepare-punk-boots.py'
FREEZE = ROOT / '_docs' / 'punk-boots-2026-09-16' / 'freeze.mjs'
RESOLVER = ROOT / 'src' / 'rig' / 'SourceAssembly.ts'
COHORT = ROOT / '_docs' / 'punk-boots-2026-09-16' / 'cohort.json'
DEFAULT_MATERIAL = ('/Game/Discovery/Characters/Streetwear/Assets/PunkBoots/'
                    'M_Streetwear_PunkBoots.M_Streetwear_PunkBoots')
# The previous (sniper-boot) family's mesh, slot, tags and morph order: stale values that must never pass here.
# Its two tags lack the wide-boot shrink rule, and its morph pair is the reverse of this mesh's source order.
SNIPER_MESH = '/Game/Discovery/Characters/Military/Assets/SniperBoots/SK_Military_SniperBoots_M.SK_Military_SniperBoots_M'
SNIPER_TAGS = ['Customization.Shape.PushInsideClothes.push_lower_feet',
               'Customization.Shape.ShrinkWrap.shrink_pants_in_lowboots']
SNIPER_MORPHS = ['medium_male', 'medium_female']
# A fitting tag of an unrelated (assault-vest) family.
FOREIGN_TAG = 'Customization.Shape.ShrinkWrap.shrink_under_collar'
...
def load_adapter():
    ...
    configure('_docs/sniper-boots-2026-09-16/batch.json', 'sniper-boots', 'sniper-boots-runtime', 'sniper-boots-preview')
    shared = types.SimpleNamespace(d=d, DOCS=Path('sniper-boots'), WORK=Path('sniper-boots'), RUNTIME=Path('sniper-boots'),
                                   PREVIEW=Path('sniper-boots'), MESH_REPORT=Path('sniper-boots'), progress=None,
    ...
    spec = real('prepare_punk_boots', ADAPTER)
...
def mesh_document(names, slot, uv_sets=2, bones=16):
...
class AdapterGuards(unittest.TestCase):
    def setUp(self):
        ...
        self.other_mesh = self.a.SOURCE_MESH.replace('PunkBoots_M', 'PunkBoots_F')
    ...
    def stage_mesh(self, slot=None, morphs=None, glb_names=None, dto_source=None, record_path=None,
        ...
        dto = folder / 'SK_Streetwear_PunkBoots_M.mesh.json'
        ...
        path = a.WORK / 'meshes' / 'SK_Streetwear_PunkBoots_M.glb'
        path.parent.mkdir(parents=True, exist_ok=True)
    ...
    def test_supplied_cohort_is_accepted_and_drift_rejected(self):
        ...
        for broken in ({**cohort, 'count': a.EXPECTED_COUNT + 1}, {**cohort, 'items': rest},
                       {**cohort, 'meshes': [self.other_mesh]}, {**cohort, 'meshes': [SNIPER_MESH]},
    ...
    def test_each_choice_keeps_exactly_the_three_boot_fitting_tags(self):
        ...
        self.assertEqual({tuple(sorted(tags)) for tags in mapping.values()},
                         {('Customization.Shape.PushInsideClothes.push_lower_feet',
                           'Customization.Shape.ShrinkWrap.shrink_pants_in_lowboots',
                           'Customization.Shape.ShrinkWrap.shrink_pants_in_wideboots')})
        self.assertTrue(a._is_fitting_tags(item, mapping[item][::-1]))  # source order is free
        tags = mapping[item]
        wide = [tag for tag in tags if tag.endswith('wideboots')]
        # The sniper-boot pair lacks the wide-boot rule; a repeated tag cannot stand in for a missing one.
        for drift in (None, [], tags[:1], SNIPER_TAGS, wide, [*SNIPER_TAGS, SNIPER_TAGS[1]], [*tags, tags[0]],
                      [*tags, FOREIGN_TAG], [*SNIPER_TAGS, FOREIGN_TAG],
                      [*SNIPER_TAGS, wide[0].replace('wideboots', 'highboots')], [tag.lower() for tag in tags]):
    ...
    def test_report_must_name_the_preserved_glb(self):
        ...
        with self.assertRaisesRegex(SystemExit, 'preserved punk-boot conversion'):
    ...
    def test_extracted_mesh_folder_is_reused_only_for_its_exact_request(self):
        ...
        a.d.write(a.d.SOURCE / 'meshes-01.requests.json', [a.d.package_of(SNIPER_MESH)])
        with self.assertRaises(SystemExit):
            a._require_mesh_request()
    ...
    def test_mesh_contract_rejects_other_mesh_slot_source_and_lost_or_misnamed_morphs(self):
        ...
        self.assertEqual((recorded['morphNames'], recorded['slots'], recorded['bones']),
                         (['medium_female', 'medium_male'], slots, 16))
        a.d.mesh_verifier.verify.return_value = {'passed': False}
        with self.assertRaisesRegex(SystemExit, 'no longer verify'):
            a.mesh()
        other, sniper = a.d.package_of(self.other_mesh), a.d.package_of(SNIPER_MESH)
        female, male = a.MORPH_NAMES
        for change, reason in (({'slot': 'SniperBoots'}, 'PunkBoots material slot'),
                               ({'slot': 'PunkBoots_Laces'}, 'PunkBoots material slot'),
                               ({'dto_source': other}, 'DTO source identity'),
                               ({'dto_source': sniper}, 'DTO source identity'),
                               ({'record_path': sniper}, 'mesh source identity'),
                               ...
                               ({'morphs': SNIPER_MORPHS}, 'not the original'),  # the sniper-boot mesh's order
                               ...
                               ({'glb_names': SNIPER_MORPHS}, 'GLB morph target names'),
                               ({'glb_names': []}, 'GLB morph target names'),
                               ({'facts': {'bones': 12}}, 'differs from the exported mesh'),  # the sniper-boot skeleton
                               ({'facts': {'vertices': 4637}}, 'differs from the exported mesh'),
                               ({'facts': {'triangles': 7443}}, 'differs from the exported mesh'),
    ...
    def test_glb_keeps_original_uv_sets_skinning_and_inactive_named_morphs(self):
        ...
        self.assertEqual(a._require_glb_geometry({**report, 'morphs': list(names), 'bones': list(range(16))}, dto), names)
    ...
    def test_frozen_cohort_keeps_one_boot_part_and_exact_fitting_tags(self):
        ...
        for changed, fields, reason in ((tagged({}), {}, drift),  # missing is not valid for a boot
                                        ...
                                        (tagged({'ActivatesTags': [*tags, FOREIGN_TAG]}), {}, drift),
                                        (tagged({'ActivatesTags': SNIPER_TAGS}, index=0), {}, drift),
                                        (tagged({'ActivatesTags': [*SNIPER_TAGS, SNIPER_TAGS[1]]}, index=0), {}, drift),
                                        ({3: {'fittingTags': tags[1:]}}, {}, drift),
                                        ({3: {'fittingTags': sorted(SNIPER_TAGS)}}, {}, drift),
                                        ({3: {'fittingTags': [*tags, tags[1]]}}, {}, drift),
                                        ({0: {'effectiveParts': [{**part, 'mesh': SNIPER_MESH}]}}, {}, 'punk-boot mesh'),
                                        ({0: {'effectiveParts': [{**part, 'slots': [{**part['slots'][0], 'slot': 'SniperBoots'}]}]}},
                                         {}, 'punk-boot mesh'),
                                        ({0: {'effectiveParts': [part, {**part, 'sourceIndex': 1}]}}, {}, 'punk-boot mesh'),
                                        ...
                                        (None, {'sourceSlots': [{**slots[0], 'slot': 'SniperBoots'}]}, 'different mesh report')):
...
// The previous (sniper-boot) family: its mesh, its two tags without the wide-boot rule, its reversed morph pair.
const SNIPER_MESH = '/Game/Discovery/Characters/Military/Assets/SniperBoots/SK_Military_SniperBoots_M.SK_Military_SniperBoots_M';
const sniperTags = ['Customization.Shape.PushInsideClothes.push_lower_feet', 'Customization.Shape.ShrinkWrap.shrink_pants_in_lowboots'];
const foreignTag = 'Customization.Shape.ShrinkWrap.shrink_under_collar';
...
assert.throws(() => m.assertCohort({...cohort, meshes: [SNIPER_MESH]}), /skeletal mesh/);
...
assert.throws(() => m.assertMeshReport({...report, sha256: 'ff'}, inventory, shaOf('ff')), /preserved punk-boot conversion/);
assert.throws(() => m.assertMeshReport(report, inventory, shaOf('ff')), /GLB hash does not match/);
assert.throws(() => m.assertMeshReport({...report, source: m.SOURCE_MESH.replaceAll('PunkBoots_M', 'PunkBoots_F')}, inventory, shaOf(m.GLB_SHA256)), /skeletal mesh/);
assert.throws(() => m.assertMeshReport({...report, source: SNIPER_MESH}, inventory, shaOf(m.GLB_SHA256)), /skeletal mesh/);
assert.throws(() => m.assertMeshReport(report, new Set([m.packageOf(SNIPER_MESH)]), shaOf(m.GLB_SHA256)), /inventory/);
for (const drift of [{bones: 12}, {uvSets: 1}, {vertices: '4638'}, {triangles: 7443}, {materialSections: 2}])
...
// Lost, reordered into the sniper-boot mesh's order, renamed, or an invented extra morph.
for (const list of [names.slice(0, 1), [], [...names].reverse(), ['medium_female', 'Medium_Male'], [...names, 'push_lower_feet']])
...
// Fitting tags: every choice has exactly the three boot tags; order is free; anything else is drift.
...
assert.deepEqual([...new Set(Object.values(m.EXPECTED_ACTIVATED_TAGS_BY_ITEM).map(row => JSON.stringify([...row].sort())))],
  [JSON.stringify(['Customization.Shape.PushInsideClothes.push_lower_feet', 'Customization.Shape.ShrinkWrap.shrink_pants_in_lowboots',
    'Customization.Shape.ShrinkWrap.shrink_pants_in_wideboots'])]);
assert.equal(m.isFittingTags('streetwear-punkboots-unknown', tags), false);
assert.equal(m.isFittingTags('military-sniperboots-leather', tags), false);
assert.equal(m.isFittingTags(id, [...tags].reverse()), true);

const latest = 'Discovery/Content/Discovery/Characters/Streetwear/Assets/PunkBoots/Skins/Leather_Brown/MI_Streetwear_PunkBoots_Leather_Brown.uasset';
const saved = latest.replace('Leather_Brown/', 'Leather_BROWN/');
const other = latest.replaceAll('Leather_Brown', 'Leather_LightBrown');
...
// The sniper-boot pair lacks the wide-boot rule; a repeated tag cannot stand in for the missing one.
for (const drift of [undefined, [], tags.slice(0, 1), sniperTags, [...sniperTags, sniperTags[1]], [...tags, tags[0]], [...tags, foreignTag],
                     [...sniperTags, tags[2].replace('wideboots', 'highboots')], tags.map(tag => tag.toLowerCase())])
...
assert.throws(() => m.resolveDefinitionPackage({...definition, source: saved.replace('MI_Streetwear_PunkBoots_Leather_Brown', 'MI_Absent')}, archive, inventoryIndex, id), /absent/);
...
const dir = fs.mkdtempSync(path.join(os.tmpdir(), '.punk-boots-freeze-'));
try {
...
const slots = [{slot: m.MATERIAL_SLOT, material: '/Game/Discovery/Characters/Streetwear/Assets/PunkBoots/M_Default.M_Default'}];
...
const LIGHT = m.SOURCE_MESH.replaceAll('PunkBoots_M', 'PunkBoots_L'), HEAVY = m.SOURCE_MESH.replaceAll('PunkBoots_M', 'PunkBoots_H');
...
const definition = {formatVersion: 1, id: item.id, source: 'Discovery/Content/PunkBoots.uasset', sourceSha256: 'ab',
...
// Medium resolves one visible part on the boot mesh with the cohort material and hands exactly the three boot
...
// Missing, empty, partial (the sniper-boot pair without the wide-boot rule) or foreign (assault-vest) activated tags are drift.
for (const tags of [undefined, [], m.tagsFor(item.id).slice(0, 1), m.tagsFor(item.id).slice(0, 2),
                    [...m.tagsFor(item.id), 'Customization.Shape.ShrinkWrap.shrink_under_collar']])
...
rejects(variant({}, {TagOverrides: [...archetypes, rule(['Customization.Shape.ShrinkWrap'], {bOverrideMesh: true, ReplacementSkeletalMesh: soft(LIGHT)})]}), /punk-boot mesh/);

const medium = fields => ({TagOverrides: [...archetypes, rule([m.CONTEXT], fields)]});
rejects(variant({}, medium({bOverrideMesh: true, ReplacementSkeletalMesh: soft(LIGHT)})), /punk-boot mesh/);
...
class FreezeContracts(unittest.TestCase):
    ...
    def test_adapter_and_freeze_share_the_handed_over_contract(self):
        ...
        # The source-index count, not the product UI's full touched total of 569.
        self.assertEqual((adapter.BASELINE_ADVERTISED, adapter.BASELINE_STRUCTURAL, adapter.BASELINE_INDEXED,
                          adapter.CATALOG_ITEMS), (522, 523, 523, 2866))
        self.assertEqual((adapter.MESH_FACTS, adapter.GLB_SHA256, adapter.MORPH_NAMES),
                         ({'vertices': 4638, 'triangles': 7442, 'uvSets': 2, 'bones': 16, 'materialSections': 1},
                          '122ca4eaaefc527b56e608e4939af6f9a66ec29705f876b1945f8532bd460537', ['medium_female', 'medium_male']))
        self.assertEqual((adapter.SOURCE_MESH, adapter.MATERIAL_SLOT, adapter.ITEM_SLOT, adapter.MARKER),
                         ('/Game/Discovery/Characters/Streetwear/Assets/PunkBoots/SK_Streetwear_PunkBoots_M.SK_Streetwear_PunkBoots_M',
                          'PunkBoots', 'feet', 'shader-probe/prepare-punk-boots'))
