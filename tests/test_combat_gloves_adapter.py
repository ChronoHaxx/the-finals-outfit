"""Focused negative contracts for the Military Combat Gloves adapter and freeze, without game assets.
...
source-only cohort.json is the only fixture; values under test come from the modules, not copies.
  ...
  C:/ProgramData/anaconda3/python.exe -B tests/test_combat_gloves_adapter.py -v
"""
import hashlib
...
ADAPTER = ROOT / 'scripts' / 'shader-probe' / 'prepare-combat-gloves.py'
FREEZE = ROOT / '_docs' / 'combat-gloves-2026-09-15' / 'freeze.mjs'
RESOLVER = ROOT / 'src' / 'rig' / 'SourceAssembly.ts'
COHORT = ROOT / '_docs' / 'combat-gloves-2026-09-15' / 'cohort.json'
DEFAULT_MATERIAL = '/Game/Discovery/Characters/Military/Assets/CombatGloves/M_Military_CombatGloves.M_Military_CombatGloves'
...
def load_adapter():
    ...
    configure('_docs/assault-gloves-2026-09-15/batch.json', 'assault', 'assault-runtime', 'assault-preview')
    shared = types.SimpleNamespace(d=d, DOCS=Path('assault'), WORK=Path('assault'), RUNTIME=Path('assault'),
                                   PREVIEW=Path('assault'), MESH_REPORT=Path('assault'), progress=None,
    ...
    spec = real('prepare_combat_gloves', ADAPTER)
...
def mesh_document(names, slot, uv_sets=2):
    ...
    return {'materials': [{'extras': {'sourceSlot': {'MaterialSlotName': slot}, 'sourceMaterial': DEFAULT_MATERIAL}}],
            ...
            'nodes': [{'mesh': 0, 'skin': 0}],
            'skins': [{'joints': list(range(53)), 'inverseBindMatrices': 7}]}
...
class AdapterGuards(unittest.TestCase):
    ...
    def stage_mesh(self, slot=None, morphs=None, glb_names=None, dto_source=None, record_path=None,
        ...
        dto = folder / 'SK_Military_CombatGloves_M.mesh.json'
        ...
        path = a.WORK / 'meshes' / 'SK_Military_CombatGloves_M.glb'
        path.parent.mkdir(parents=True, exist_ok=True)
    ...
    def test_every_shared_global_and_hook_is_repointed(self):
        ...
        for name, stale in (('REUSE_TEXTURES', [Path('scripts/generated/shader-probe/accessory-source-v1/opus-textures-01')]),
                            ...
                            ('MARKER', 'shader-probe/prepare-assault-gloves'), ('plan', object())):
            original = getattr(a.d, name)
            setattr(a.d, name, stale)
            with self.assertRaises(SystemExit):
                a._require_configuration()
            setattr(a.d, name, original)
        for name, stale in (('MESH_REPORT', Path('_docs/assault-gloves-2026-09-15/mesh-report.json')), ('progress', print)):
    ...
    def test_report_must_name_the_preserved_glb(self):
        ...
        with self.assertRaisesRegex(SystemExit, 'preserved combat-gloves conversion'):
    ...
    def test_mesh_contract_rejects_other_mesh_slot_source_and_lost_or_misnamed_morphs(self):
        ...
        self.assertEqual((recorded['morphNames'], recorded['slots'], recorded['bones']), (a.MORPH_NAMES, slots, 53))
        ...
        medium, shrink = a.MORPH_NAMES[:2], a.MORPH_NAMES[2:]
        for change, reason in (({'slot': 'AssaultGloves'}, 'CombatGloves material slot'),
                               ...
                               ({'morphs': medium}, 'not the original'),  # the fingerless glove's pair only
                               ({'morphs': [*medium, shrink[0]]}, 'not the original'),
                               ({'morphs': []}, 'not the original'),
                               ({'morphs': [*medium, *shrink[::-1]]}, 'not the original'),
                               ({'morphs': [*medium, shrink[0], 'shrink_gloves_under_jacket_left']}, 'not the original'),
                               ({'morphs': [*medium, shrink[0], 'Shrink_Gloves_Under_Jacket_Right']}, 'not the original'),
                               ({'morphs': [*a.MORPH_NAMES, 'medium_extra']}, 'not the original'),
                               # Lost or misnamed only by the conversion.
                               ({'glb_names': [*medium, shrink[0]]}, 'GLB morph target names'),
                               ({'glb_names': [*medium, shrink[0], 'shrink_gloves_under_jacket_r']}, 'GLB morph target names'),
                               ({'glb_names': []}, 'GLB morph target names'),
                               ({'facts': {'bones': 46}}, 'differs from the exported mesh'),
                               ({'facts': {'vertices': 5675}}, 'differs from the exported mesh'),
    ...
    def test_glb_keeps_original_uv_sets_skinning_and_inactive_named_morphs(self):
        ...
        self.assertEqual(a._require_glb_geometry({**report, 'morphs': list(names), 'bones': list(range(53))}, dto), names)
        ...
        for broken_mesh in ({**mesh, 'extras': {}},                                          # names lost
                            {**mesh, 'extras': {'targetNames': [*names[:3], 'renamed']}},
        ...
        # A sleeve-fitting morph switched on by default (mesh or node weights) would shrink every glove.
        for activated in ({**document, 'meshes': [{**mesh, 'weights': [0, 0, 1, 0]}]},
                          {**document, 'nodes': [{'mesh': 0, 'skin': 0, 'weights': [0, 0, 0, 0.5]}]}):
    ...
    def test_frozen_cohort_keeps_one_gloves_part_and_exactly_the_fitting_tags(self):
        ...
        part, assault = rows[0]['effectiveParts'][0], {'slot': 'AssaultGloves', 'material': rows[0]['materials'][0]}
        for first, fields, reason in ((tagged({}), {}, 'does not activate exactly'),  # missing is not valid for full gloves
                                      ...
                                      (tagged({'ActivatesTags': [tags[0], tags[0]]}), {}, 'does not activate exactly'),
                                      (tagged({'ActivatesTags': [*tags, 'Customization.Fitting.Gloves']}), {}, 'does not activate exactly'),
                                      (tagged({'ActivatesTags': [tags[0], tags[1] + '_right']}), {}, 'does not activate exactly'),
                                      ({'fittingTags': []}, {}, 'does not activate exactly'),
                                      ({'fittingTags': tags[1:]}, {}, 'does not activate exactly'),
                                      ({'effectiveParts': [{**part, 'mesh': self.other_mesh}]}, {}, 'combat-gloves mesh'),
                                      ({'effectiveParts': [{**part, 'slots': [assault]}]}, {}, 'combat-gloves mesh'),
                                      ({'effectiveParts': [part, {**part, 'sourceIndex': 1}]}, {}, 'combat-gloves mesh'),
                                      ...
                                      ({}, {'sourceSlots': [{**slots[0], 'slot': 'AssaultGloves'}]}, 'different mesh report')):
            stage(first, **fields)
    ...
    def test_preview_keeps_the_baseline_progress_and_its_unadvertised_entry(self):
        ...
        for broken in ({'supported': advertised + implemented + ['knight-pants']},
                       ...
                       {'preview': {**preview, 'marker': 'shader-probe/prepare-assault-gloves'}},
                       {'preview': {**preview, 'implemented': ['military-assaultgloves-x']}}):
            stage(**broken)
            with self.assertRaises(SystemExit):
                a._require_preview()
...
assert.throws(() => m.assertMeshReport({...report, sha256: 'ff'}, inventory, shaOf('ff')), /preserved combat-gloves conversion/);
...
assert.throws(() => m.assertMeshReport({...report, source: m.SOURCE_MESH.replaceAll('CombatGloves', 'AssaultGloves')}, inventory, shaOf(m.GLB_SHA256)), /skeletal mesh/);
assert.throws(() => m.assertMeshReport(report, new Set(), shaOf(m.GLB_SHA256)), /inventory/);
for (const drift of [{bones: 46}, {uvSets: 1}, {vertices: '5676'}, {triangles: 9593}, {materialSections: 2}])
...
assert.throws(() => m.sourceSlotsFromDto({sourceMaterials: [{MaterialSlotName: 'AssaultGloves'}], materials: [{path: '/Game/M.M'}]}), /material slot must be/);
...
// Morph targets are the four original Medium morphs by name, order and count, inactive by default.
const names = m.MORPH_NAMES;
assert.deepEqual(names, ['medium_male', 'medium_female', 'shrink_gloves_under_jacket', 'shrink_gloves_under_jacket_right']);
...
m.assertGlbMorphs({...glbDoc, meshes: [{...glbDoc.meshes[0], weights: [0, 0, 0, 0]}]}, names, report);
assert.throws(() => m.morphNamesFromDto(dtoFor([...names.slice(0, 3), names[2]])), /not unique/);
for (const list of [names.slice(0, 2), names.slice(0, 3), [], [...names].reverse(), [...names.slice(0, 2), names[3], names[2]],
                    [...names.slice(0, 3), 'shrink_gloves_under_jacket_left'], [...names, 'medium_extra']])
  assert.throws(() => m.morphNamesFromDto(dtoFor(list)), /original Medium morphs/);
assert.throws(() => m.assertGlbMorphs(glbDoc, names, {...report, morphs: 2}), /morph count/);
assert.throws(() => m.assertGlbMorphs({meshes: [{primitives: [primitive]}]}, names, report), /target names/);
assert.throws(() => m.assertGlbMorphs({meshes: [{extras: {targetNames: [...names.slice(0, 3), 'renamed']}, primitives: [primitive]}]}, names, report), /target names/);
...
assert.throws(() => m.assertGlbMorphs({...glbDoc, meshes: [{...glbDoc.meshes[0], weights: [0, 0, 1, 0]}]}, names, report), /by default/);
assert.throws(() => m.assertGlbMorphs({...glbDoc, nodes: [{mesh: 0, weights: [0, 0, 0, 1]}]}, names, report), /by default/);
...
const latest = 'Discovery/Content/Discovery/Characters/Military/Assets/CombatGloves/Skins/Folder_Case/MI_Gloves.uasset';
...
// Both full-glove tags in any order; missing, empty, partial, duplicated, extra or renamed tags are drift.
assert.deepEqual(tags, ['Customization.HideMesh.NailsCovered', 'Customization.Shape.PushInsideClothes.push_gloves']);
m.assertDefinition({id: 'x'}, definition);
m.assertDefinition({id: 'x'}, {...definition, properties: {...definition.properties, ActivatesTags: [...tags].reverse()}});
for (const drift of [undefined, [], tags.slice(0, 1), [tags[0], tags[0]], [...tags, 'Customization.Fitting.Gloves'],
                     [tags[0], tags[1] + '_right'], tags.map(tag => tag.toLowerCase())])
...
assert.throws(() => m.assertResolved(item, definition, {parts: [{...part, skeletalMesh: m.SOURCE_MESH.replace(/_M\b/g, '_F')}]}, slots), /combat-gloves mesh/);
...
const dir = fs.mkdtempSync(path.join(os.tmpdir(), '.combat-gloves-freeze-'));
try {
...
const slots = [{slot: m.MATERIAL_SLOT, material: '/Game/Discovery/Characters/Military/Assets/CombatGloves/M_Default.M_Default'}];
...
// Medium resolves one visible part on the gloves mesh with the cohort material and hands both fitting tags
...
// Missing, empty, partial or extra activated tags are drift for a full glove.
for (const tags of [undefined, [], m.FITTING_TAGS.slice(0, 1), [...m.FITTING_TAGS, 'Customization.Fitting.Gloves']])
...
rejects(variant({}, {TagOverrides: [...archetypes, rule(['Customization.Shape.PushInsideClothes'], {bOverrideMesh: true, ReplacementSkeletalMesh: soft(LIGHT)})]}), /combat-gloves mesh/);

const medium = fields => ({TagOverrides: [...archetypes, rule([m.CONTEXT], fields)]});
rejects(variant({}, medium({bOverrideMesh: true, ReplacementSkeletalMesh: soft(LIGHT)})), /combat-gloves mesh/);
...
rejects(variant({MaterialOverrides: [{Key: 'AssaultGloves', Value: soft(material)}]}), /absent slot/);
...
class FreezeContracts(unittest.TestCase):
    ...
    def test_adapter_and_freeze_share_the_handed_over_contract(self):
        ...
        # The source-index count, not the product UI's full touched total of 517.
        self.assertEqual((adapter.BASELINE_ADVERTISED, adapter.BASELINE_STRUCTURAL, adapter.BASELINE_INDEXED,
                          adapter.CATALOG_ITEMS), (470, 471, 471, 2866))
