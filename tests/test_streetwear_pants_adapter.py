"""Focused negative contracts for the Streetwear Pants adapter and freeze, without game assets.
...
source-only cohort.json is the only fixture; values under test come from the modules, not copies.
  ...
  C:/ProgramData/anaconda3/python.exe -B tests/test_streetwear_pants_adapter.py -v
"""
import hashlib
...
ADAPTER = ROOT / 'scripts' / 'shader-probe' / 'prepare-streetwear-pants.py'
FREEZE = ROOT / '_docs' / 'streetwear-pants-2026-09-16' / 'freeze.mjs'
RESOLVER = ROOT / 'src' / 'rig' / 'SourceAssembly.ts'
COHORT = ROOT / '_docs' / 'streetwear-pants-2026-09-16' / 'cohort.json'
DEFAULT_MATERIAL = ('/Game/Discovery/Characters/Streetwear/Assets/Pants/'
                    'M_Streetwear_Pants.M_Streetwear_Pants')
# The previous (sniper-boot) family's mesh, slot and tags: stale values that must never pass here.
BOOT_MESH = '/Game/Discovery/Characters/Military/Assets/SniperBoots/SK_Military_SniperBoots_M.SK_Military_SniperBoots_M'
BOOT_SLOT = 'SniperBoots'
BOOT_TAGS = ['Customization.Shape.PushInsideClothes.push_lower_feet',
             'Customization.Shape.ShrinkWrap.shrink_pants_in_lowboots']
REVERSED_MORPHS = ['medium_female', 'medium_male']
...
def load_adapter():
    ...
    configure('_docs/sniper-boots-2026-09-16/batch.json', 'boots', 'boots-runtime', 'boots-preview')
    shared = types.SimpleNamespace(d=d, DOCS=Path('boots'), WORK=Path('boots'), RUNTIME=Path('boots'),
                                   PREVIEW=Path('boots'), MESH_REPORT=Path('boots'), progress=None,
    ...
    spec = real('prepare_streetwear_pants', ADAPTER)
...
def mesh_document(names, slot, uv_sets=1, bones=22):
...
class AdapterGuards(unittest.TestCase):
    def setUp(self):
        ...
        self.other_mesh = self.a.SOURCE_MESH.replace('Pants_M', 'Pants_F')
    ...
    def stage_mesh(self, slot=None, morphs=None, glb_names=None, dto_source=None, record_path=None,
        ...
        dto = folder / 'SK_Streetwear_Pants_M.mesh.json'
        ...
        path = a.WORK / 'meshes' / 'SK_Streetwear_Pants_M.glb'
        path.parent.mkdir(parents=True, exist_ok=True)
    ...
    def test_every_shared_global_and_hook_is_repointed(self):
        ...
        for name, stale in (('REUSE_TEXTURES', [Path('scripts/generated/shader-probe/sniper-boots-v1/opus-textures-01')]),
                            ('MESH_EVIDENCE', [Path('scripts/generated/shader-probe/sniper-boots-v1/meshes-01')]),
                            ('MARKER', 'shader-probe/prepare-sniper-boots'), ('plan', object())):
            original = getattr(a.d, name)
            setattr(a.d, name, stale)
            with self.assertRaises(SystemExit):
                a._require_configuration()
            setattr(a.d, name, original)
        for name, stale in (('MESH_REPORT', Path('_docs/sniper-boots-2026-09-16/mesh-report.json')), ('progress', print)):
    ...
    def test_supplied_cohort_is_accepted_and_drift_rejected(self):
        ...
        for broken in ({**cohort, 'count': a.EXPECTED_COUNT + 1}, {**cohort, 'items': rest},
                       {**cohort, 'meshes': [self.other_mesh]}, {**cohort, 'meshes': [BOOT_MESH]},
    ...
    def test_each_choice_keeps_exactly_the_three_push_inside_tags(self):
        ...
        self.assertEqual({tuple(sorted(tags)) for tags in mapping.values()},
                         {('Customization.Shape.PushInsideClothes.push_right_calf',
                           'Customization.Shape.PushInsideClothes.push_thighs',
                           'Customization.Shape.PushInsideClothes.push_upper_thighs')})
        self.assertTrue(a._is_fitting_tags(item, mapping[item][::-1]))  # source order is free
        tags = mapping[item]
        for drift in (None, [], tags[:2], [tags[0], tags[0], tags[1]], [*tags, tags[0]], [*tags, BOOT_TAGS[0]], BOOT_TAGS,
                      [*tags[:2], tags[2].replace('upper_thighs', 'lower_thighs')], [tag.lower() for tag in tags]):
            self.assertFalse(a._is_fitting_tags(item, drift), drift)
        self.assertFalse(a._is_fitting_tags(item + '-v2', tags))
        self.assertFalse(a._is_fitting_tags('military-sniperboots-leather', tags))
    ...
    def test_report_must_name_the_preserved_glb(self):
        ...
        with self.assertRaisesRegex(SystemExit, 'preserved streetwear-pants conversion'):
    ...
    def test_extracted_mesh_folder_is_reused_only_for_its_exact_request(self):
        ...
        a.d.write(a.d.SOURCE / 'meshes-01.requests.json', [a.d.package_of(BOOT_MESH)])
        with self.assertRaises(SystemExit):
            a._require_mesh_request()
    ...
    def test_mesh_contract_rejects_other_mesh_slot_source_and_lost_or_misnamed_morphs(self):
        ...
        self.assertEqual((recorded['morphNames'], recorded['slots'], recorded['bones'], recorded['uvSets']),
                         (['medium_male', 'medium_female'], slots, 22, 1))
        a.d.mesh_verifier.verify.return_value = {'passed': False}
        with self.assertRaisesRegex(SystemExit, 'no longer verify'):
            a.mesh()
        other, boot = a.d.package_of(self.other_mesh), a.d.package_of(BOOT_MESH)
        male, female = a.MORPH_NAMES
        for change, reason in (({'slot': BOOT_SLOT}, 'Pants material slot'),
                               ({'slot': 'Pants_Belt'}, 'Pants material slot'),
                               ({'dto_source': other}, 'DTO source identity'),
                               ({'record_path': boot}, 'mesh source identity'),
                               ...
                               ({'morphs': REVERSED_MORPHS}, 'not the original'),
                               ({'morphs': [male, 'Medium_Female']}, 'not the original'),
                               # The push tags add no cuff or calf morph to this mesh.
                               ({'morphs': [*a.MORPH_NAMES, 'push_right_calf']}, 'not the original'),
                               ...
                               ({'glb_names': REVERSED_MORPHS}, 'GLB morph target names'),
                               ({'glb_names': []}, 'GLB morph target names'),
                               ({'facts': {'bones': 12}}, 'differs from the exported mesh'),  # the sniper-boot skeleton
                               ({'facts': {'vertices': 2171}}, 'differs from the exported mesh'),
                               ({'facts': {'triangles': 3674}}, 'differs from the exported mesh'),
                               ({'facts': {'materialSections': 2}}, 'differs from the exported mesh'),
                               ({'facts': {'uvSets': 2}}, 'differs from the exported mesh')):
            self.stage_mesh(**change)
            with self.assertRaisesRegex(SystemExit, reason, msg=change):
                a._require_mesh_contract()

    def test_glb_keeps_its_one_uv_set_skinning_and_inactive_named_morphs(self):
        ...
        self.assertEqual(sorted(k for k in attributes if k.startswith('TEXCOORD_')), ['TEXCOORD_0'])
        ...
        self.assertEqual(a._require_glb_geometry({**report, 'morphs': list(names), 'bones': list(range(22))}, dto), names)
        path.write_bytes(glb({**document, 'meshes': [{**mesh, 'weights': [0] * len(names)}]}))  # explicit zero defaults
        self.assertEqual(a._require_glb_geometry(report, dto), names)
        for broken_report in ({**report, 'uvSets': 2}, {**report, 'uvSets': 0}, {**report, 'uvSets': True},
                              {**report, 'bones': 0}, {**report, 'morphs': len(names) - 1}, {**report, 'morphs': None},
        ...
        for broken_mesh in ({**mesh, 'extras': {}},                                          # names lost
                            ...
                            # A fabricated second UV set, or the only original one lost.
                            {**mesh, 'primitives': [{**primitive, 'attributes': {**attributes, 'TEXCOORD_1': 2}}]},
                            {**mesh, 'primitives': [{**primitive, 'attributes': {k: v for k, v in attributes.items() if k != 'TEXCOORD_0'}}]},
        ...
        # A body morph switched on by default (mesh or node weights) would reshape every pair of pants.
    ...
    def test_frozen_cohort_keeps_one_pants_part_and_exact_fitting_tags(self):
        a, cohort = self.a, self.cohort
        ...
        for changed, fields, reason in ((tagged({}), {}, drift),  # missing is not valid for these pants
                                        (tagged({'ActivatesTags': []}), {}, drift),
                                        (tagged({'ActivatesTags': tags[:2]}), {}, drift),
                                        (tagged({'ActivatesTags': [tags[0], tags[0], tags[1]]}), {}, drift),
                                        (tagged({'ActivatesTags': [*tags, BOOT_TAGS[1]]}), {}, drift),
                                        (tagged({'ActivatesTags': BOOT_TAGS}, index=0), {}, drift),
                                        ({3: {'fittingTags': tags[1:]}}, {}, drift),
                                        ({3: {'fittingTags': [*tags, tags[1]]}}, {}, drift),
                                        ({0: {'effectiveParts': [{**part, 'mesh': BOOT_MESH}]}}, {}, 'streetwear-pants mesh'),
                                        ({0: {'effectiveParts': [{**part, 'slots': [{**part['slots'][0], 'slot': BOOT_SLOT}]}]}},
                                         {}, 'streetwear-pants mesh'),
                                        ({0: {'effectiveParts': [part, {**part, 'sourceIndex': 1}]}}, {}, 'streetwear-pants mesh'),
                                        ({0: {'slot': 'feet'}}, {}, 'differs from cohort.json'),
                                        (None, {'context': 'Customization.Archetype.Heavy'}, 'not resolved for'),
                                        (None, {'meshes': [self.other_mesh]}, 'different mesh report'),
                                        (None, {'sourceSlots': [{**slots[0], 'slot': BOOT_SLOT}]}, 'different mesh report')):
    ...
    def test_shader_needing_a_second_uv_set_is_blocked_on_this_one_uv_mesh(self):
        a = self.a
        a._require_mesh_contract = lambda: ({'uvSets': a.MESH_FACTS['uvSets']}, [])
        ...
        a.d.write(a.RUNTIME / 'staging' / 'mi-two.json', {})  # an undeclared requirement keeps the [0, 1] default
        a.d.write(a.RUNTIME / 'staging' / 'mi-three.json', {'requiredUvSets': [0, 1]})
        self.assertEqual(sorted(a.geometry_blocked()), ['mi-three', 'mi-two'])
        records = {r['itemId']: r for r in a.d.read_json(a.WORK / 'geometry-contracts.json')}
        self.assertEqual(records['mi-one']['availableUvSets'], [0])
        self.assertEqual(records['mi-one']['blockers'], [])
    ...
    def test_preview_keeps_the_baseline_progress_and_its_unadvertised_entry(self):
        ...
        for broken in ({'supported': advertised + implemented + ['knight-pants']},
                       ...
                       {'preview': {**preview, 'previousAdvertised': 502, 'previousAssemblies': 503}},
                       {'preview': {**preview, 'previewAssemblies': a.BASELINE_STRUCTURAL + 2}},
                       {'preview': {**preview, 'marker': 'shader-probe/prepare-sniper-boots'}},
                       {'preview': {**preview, 'implemented': ['military-sniperboots-leather']}}):
            stage(**broken)
            with self.assertRaises(SystemExit):
                a._require_preview()
...
const BOOT_MESH = '/Game/Discovery/Characters/Military/Assets/SniperBoots/SK_Military_SniperBoots_M.SK_Military_SniperBoots_M';
const bootTags = ['Customization.Shape.PushInsideClothes.push_lower_feet', 'Customization.Shape.ShrinkWrap.shrink_pants_in_lowboots'];
...
assert.throws(() => m.assertCohort({...cohort, meshes: [BOOT_MESH]}), /skeletal mesh/);
...
assert.throws(() => m.assertMeshReport({...report, sha256: 'ff'}, inventory, shaOf('ff')), /preserved streetwear-pants conversion/);
assert.throws(() => m.assertMeshReport(report, inventory, shaOf('ff')), /GLB hash does not match/);
assert.throws(() => m.assertMeshReport({...report, source: m.SOURCE_MESH.replaceAll('Pants_M', 'Pants_F')}, inventory, shaOf(m.GLB_SHA256)), /skeletal mesh/);
assert.throws(() => m.assertMeshReport({...report, source: BOOT_MESH}, inventory, shaOf(m.GLB_SHA256)), /skeletal mesh/);
assert.throws(() => m.assertMeshReport(report, new Set([m.packageOf(BOOT_MESH)]), shaOf(m.GLB_SHA256)), /inventory/);
// The sniper-boot skeleton, a second (fabricated) UV set, a stringly count and near-miss geometry.
for (const drift of [{bones: 12}, {uvSets: 2}, {uvSets: 0}, {vertices: '2172'}, {triangles: 3674}, {materialSections: 2}])
...
assert.throws(() => m.sourceSlotsFromDto({sourceMaterials: [{MaterialSlotName: 'SniperBoots'}], materials: [{path: '/Game/M.M'}]}), /material slot must be/);
...
// Lost, reordered, renamed, or an invented calf morph (the push tags add none to this mesh).
for (const list of [names.slice(0, 1), [], [...names].reverse(), ['medium_male', 'Medium_Female'], [...names, 'push_right_calf']])
...
// Fitting tags: every choice has exactly the three push-inside tags; order is free; anything else is drift.
...
assert.deepEqual([...new Set(Object.values(m.EXPECTED_ACTIVATED_TAGS_BY_ITEM).map(row => JSON.stringify([...row].sort())))],
  [JSON.stringify(['Customization.Shape.PushInsideClothes.push_right_calf', 'Customization.Shape.PushInsideClothes.push_thighs',
    'Customization.Shape.PushInsideClothes.push_upper_thighs'])]);
assert.equal(m.isFittingTags('streetwear-pants-cotton-unknown', tags), false);
assert.equal(m.isFittingTags(id, [...tags].reverse()), true);

const latest = 'Discovery/Content/Discovery/Characters/Streetwear/Assets/Pants/Skins/Cotton_Black/MI_Pants_Cotton_Black.uasset';
const saved = latest.replace('Cotton_Black/', 'Cotton_BLACK/');
const other = latest.replaceAll('Cotton_Black', 'Cotton_Blue');
const archive = m.foldedIndex([latest, other]), inventoryIndex = m.foldedIndex(new Set([latest, other]));
const definition = {formatVersion: 1, id, source: saved, sourceSha256: 'ab', properties: {Slots: ['LowerBody'], ActivatesTags: tags}};
const current = {status: 'ok', package: {path: latest, sha256: 'ab'},
  exports: [{type: 'CharacterCustomizationItem', properties: {Slots: ['LowerBody'], ActivatesTags: tags}}]};
...
for (const drift of [undefined, [], tags.slice(0, 2), [tags[0], tags[0], tags[1]], [...tags, tags[0]], [...tags, bootTags[0]], bootTags,
                     [...tags.slice(0, 2), tags[2].replace('upper_thighs', 'lower_thighs')], tags.map(tag => tag.toLowerCase())])
  assert.throws(() => m.assertDefinition({id}, {...definition, properties: {Slots: ['LowerBody'], ActivatesTags: drift}}), /activated tags/);
...
assert.throws(() => m.assertCurrentDefinition(definition, definition,
  {...current, exports: [{type: 'CharacterCustomizationItem', properties: {Slots: ['LowerBody'], ActivatesTags: tags.slice(1)}}]}), /source properties changed/);
assert.throws(() => m.resolveDefinitionPackage({...definition, source: saved.replace('MI_Pants_Cotton_Black', 'MI_Absent')}, archive, inventoryIndex, id), /absent/);
...
const dir = fs.mkdtempSync(path.join(os.tmpdir(), '.streetwear-pants-freeze-'));
try {
...
const slots = [{slot: m.MATERIAL_SLOT, material: '/Game/Discovery/Characters/Streetwear/Assets/Pants/M_Default.M_Default'}];
...
const LIGHT = m.SOURCE_MESH.replaceAll('Pants_M', 'Pants_L'), HEAVY = m.SOURCE_MESH.replaceAll('Pants_M', 'Pants_H');
...
const definition = {formatVersion: 1, id: item.id, source: 'Discovery/Content/Pants.uasset', sourceSha256: 'ab',
  properties: {Slots: ['LowerBody'], ActivatesTags: [...m.tagsFor(item.id)].reverse(),
...
// Medium resolves one visible part on the pants mesh with the cohort material and hands exactly the three
// push-inside tags to the production fitting logic; Light/Heavy overrides stay outside.
...
// Missing, empty, partial or foreign (sniper-boot) activated tags are drift for these pants.
for (const tags of [undefined, [], m.tagsFor(item.id).slice(0, 2), [...m.tagsFor(item.id), 'Customization.Shape.ShrinkWrap.shrink_pants_in_lowboots']])
...
rejects(variant({}, {TagOverrides: [...archetypes, rule(['Customization.Shape.PushInsideClothes.push_thighs'], {bOverrideMesh: true})]}), /hidden/);
rejects(variant({}, {TagOverrides: [...archetypes, rule(['Customization.Shape.PushInsideClothes'], {bOverrideMesh: true, ReplacementSkeletalMesh: soft(LIGHT)})]}), /streetwear-pants mesh/);

const medium = fields => ({TagOverrides: [...archetypes, rule([m.CONTEXT], fields)]});
rejects(variant({}, medium({bOverrideMesh: true, ReplacementSkeletalMesh: soft(LIGHT)})), /streetwear-pants mesh/);
...
rejects(variant({MaterialOverrides: [{Key: 'SniperBoots', Value: soft(material)}]}), /absent slot/);
...
class FreezeContracts(unittest.TestCase):
    ...
    def test_adapter_and_freeze_share_the_handed_over_contract(self):
        ...
        # The source-index count, not the product UI's full touched total of 559.
        self.assertEqual((adapter.BASELINE_ADVERTISED, adapter.BASELINE_STRUCTURAL, adapter.BASELINE_INDEXED,
                          adapter.CATALOG_ITEMS), (512, 513, 513, 2866))
        self.assertEqual((adapter.MESH_FACTS, adapter.GLB_SHA256, adapter.MORPH_NAMES),
                         ({'vertices': 2172, 'triangles': 3673, 'uvSets': 1, 'bones': 22, 'materialSections': 1},
                          '06769c4645f87fd5500f0aaafd26dc9136acef1a59e0950b55a2b4031fa93581', ['medium_male', 'medium_female']))
        self.assertEqual((adapter.SOURCE_MESH, adapter.MATERIAL_SLOT, adapter.ITEM_SLOT, adapter.MARKER),
                         ('/Game/Discovery/Characters/Streetwear/Assets/Pants/SK_Streetwear_Pants_M.SK_Streetwear_Pants_M',
                          'Pants', 'lowerBody', 'shader-probe/prepare-streetwear-pants'))
        self.assertEqual((adapter.DOCS, adapter.WORK, adapter.RUNTIME, adapter.PREVIEW),
                         (Path('_docs/streetwear-pants-2026-09-16'), Path('scripts/generated/shader-probe/streetwear-pants-v1'),
                          Path('public/models/reconstructed-streetwear-pants-v1'),
                          Path('public/models/reconstructed-streetwear-pants-preview-v1')))
    ...
    def test_production_resolver_on_medium_admits_one_pants_part_with_its_fitting_tags(self):
