"""Focused negative contracts for the Military Assault Gloves adapter and freeze, without game assets.
...
source-only cohort.json is the only fixture; values under test come from the modules, not copies.
  ...
  C:/ProgramData/anaconda3/python.exe -B tests/test_assault_gloves_adapter.py -v
"""
import hashlib
...
ADAPTER = ROOT / 'scripts' / 'shader-probe' / 'prepare-assault-gloves.py'
FREEZE = ROOT / '_docs' / 'assault-gloves-2026-09-15' / 'freeze.mjs'
RESOLVER = ROOT / 'src' / 'rig' / 'SourceAssembly.ts'
COHORT = ROOT / '_docs' / 'assault-gloves-2026-09-15' / 'cohort.json'
DEFAULT_MATERIAL = '/Game/Discovery/Characters/Military/Assets/AssaultGloves/M_Military_AssaultGloves.M_Military_AssaultGloves'
...
def load_adapter():
    ...
    configure('_docs/assault-pants-2026-09-15/batch.json', 'pants', 'pants-runtime', 'pants-preview')
    shared = types.SimpleNamespace(d=d, DOCS=Path('pants'), WORK=Path('pants'), RUNTIME=Path('pants'),
                                   PREVIEW=Path('pants'), MESH_REPORT=Path('pants'), progress=None,
    ...
    spec = real('prepare_assault_gloves', ADAPTER)
...
def mesh_document(names, slot, uv_sets=2):
    ...
    return {'materials': [{'extras': {'sourceSlot': {'MaterialSlotName': slot}, 'sourceMaterial': DEFAULT_MATERIAL}}],
            ...
            'skins': [{'joints': list(range(46)), 'inverseBindMatrices': 7}]}
...
class AdapterGuards(unittest.TestCase):
    ...
    def stage_mesh(self, slot=None, morphs=None, glb_names=None, dto_source=None, record_path=None,
        ...
        dto = folder / 'SK_Military_AssaultGloves_M.mesh.json'
        ...
        path = a.WORK / 'meshes' / 'SK_Military_AssaultGloves_M.glb'
        path.parent.mkdir(parents=True, exist_ok=True)
    ...
    def test_every_shared_global_and_hook_is_repointed(self):
        ...
        for name, stale in (('REUSE_TEXTURES', [Path('scripts/generated/shader-probe/accessory-source-v1/opus-textures-01')]),
                            ...
                            ('MARKER', 'shader-probe/prepare-assault-pants'), ('plan', object())):
            original = getattr(a.d, name)
            setattr(a.d, name, stale)
            with self.assertRaises(SystemExit):
                a._require_configuration()
            setattr(a.d, name, original)
        for name, stale in (('MESH_REPORT', Path('_docs/assault-pants-2026-09-15/mesh-report.json')), ('progress', print)):
    ...
    def test_report_must_name_the_preserved_glb(self):
        ...
        with self.assertRaisesRegex(SystemExit, 'preserved assault-gloves conversion'):
    ...
    def test_mesh_contract_rejects_other_mesh_slot_source_and_lost_or_misnamed_morphs(self):
        ...
        self.assertEqual((recorded['morphNames'], recorded['slots'], recorded['bones']), (a.MORPH_NAMES, slots, 46))
        ...
        for change, reason in (({'slot': 'AssaultPants'}, 'AssaultGloves material slot'),
                               ...
                               ({'morphs': ['medium_male']}, 'not the original'),
                               ({'morphs': []}, 'not the original'),
                               ({'morphs': ['medium_female', 'medium_male']}, 'not the original'),
                               ({'morphs': [*a.MORPH_NAMES, 'medium_extra']}, 'not the original'),
                               ({'morphs': ['medium_male', 'Medium_Female']}, 'not the original'),
                               # Lost or misnamed only by the conversion.
                               ({'glb_names': ['medium_male', 'medium_fem']}, 'GLB morph target names'),
                               ({'glb_names': []}, 'GLB morph target names'),
                               ({'facts': {'bones': 45}}, 'differs from the exported mesh'),
                               ({'facts': {'vertices': 2803}}, 'differs from the exported mesh'),
    ...
    def test_glb_keeps_original_uv_sets_skinning_and_named_morphs(self):
        ...
        self.assertEqual(a._require_glb_geometry({**report, 'morphs': list(names), 'bones': list(range(46))}, dto), names)
    ...
    def test_frozen_cohort_keeps_one_gloves_part_and_activates_no_tags(self):
        a, cohort = self.a, self.cohort
        ...
        for item in cohort['items']:
            ...
            rows.append({**item, 'definition': definition, 'definitionFileSha256': a.d.file_sha(file), 'fittingTags': [],
        ...
        tagged = lambda tags: {'definition': {**rows[0]['definition'], 'properties': {'ActivatesTags': tags}}}
        stage(tagged([])); a._require_frozen_cohort(report, slots)  # an explicit empty list is the same as missing
        part, pants = rows[0]['effectiveParts'][0], {'slot': 'AssaultPants', 'material': rows[0]['materials'][0]}
        for first, fields, reason in ((tagged(['Customization.Fitting.Gloves']), {}, 'activates tags'),
                                      ({'fittingTags': ['Customization.Fitting.Gloves']}, {}, 'activates tags'),
                                      ({'effectiveParts': [{**part, 'mesh': self.other_mesh}]}, {}, 'assault-gloves mesh'),
                                      ({'effectiveParts': [{**part, 'slots': [pants]}]}, {}, 'assault-gloves mesh'),
                                      ({'effectiveParts': [part, {**part, 'sourceIndex': 1}]}, {}, 'assault-gloves mesh'),
                                      ...
                                      ({}, {'sourceSlots': [{**slots[0], 'slot': 'AssaultPants'}]}, 'different mesh report')):
            stage(first, **fields)
    ...
    def test_index_needs_the_adapter_baseline_not_the_preflight_evidence(self):
        a = self.a
        a._require_downstream = lambda: None
        a._require_preview = lambda: None
        a._apply_coverage_policy = mock.Mock()  # s.index is a stub in this baseline-only contract.
        ...
        a._apply_coverage_policy.assert_called_once()
    ...
    def test_preview_keeps_the_baseline_progress_and_its_unadvertised_entry(self):
        ...
        for broken in ({'supported': advertised + implemented + ['knight-pants']},
                       ...
                       {'preview': {**preview, 'marker': 'shader-probe/prepare-assault-pants'}},
                       {'preview': {**preview, 'implemented': ['military-assaultpants-x']}}):
            stage(**broken)
            with self.assertRaises(SystemExit):
                a._require_preview()
...
assert.throws(() => m.assertMeshReport({...report, sha256: 'ff'}, inventory, shaOf('ff')), /preserved assault-gloves conversion/);
...
for (const drift of [{bones: 45}, {uvSets: 1}, {vertices: '2804'}, {triangles: 4881}, {materialSections: 2}])
...
assert.throws(() => m.sourceSlotsFromDto({sourceMaterials: [{MaterialSlotName: 'AssaultPants'}], materials: [{path: '/Game/M.M'}]}), /material slot must be/);
...
// Morph targets are the original Medium pair by name, order and count; lost, renamed or dropped targets fail.
...
assert.throws(() => m.morphNamesFromDto(dtoFor(['medium_male', 'medium_male'])), /not unique/);
for (const list of [['medium_male'], [], [...names].reverse(), ['medium_male', 'Medium_Female'], [...names, 'medium_extra']])
...
assert.throws(() => m.assertGlbMorphs({meshes: [{extras: {targetNames: ['medium_male', 'renamed']}, primitives: [primitive]}]}, names, report), /target names/);
...
const latest = 'Discovery/Content/Discovery/Characters/Military/Assets/AssaultGloves/Skins/Folder_Case/MI_Gloves.uasset';
...
// A missing ActivatesTags is the same as an empty list; any activated tag is rejected.
m.assertDefinition({id: 'x'}, definition);
m.assertDefinition({id: 'x'}, {...definition, properties: {...definition.properties, ActivatesTags: []}});
assert.throws(() => m.assertDefinition({id: 'x'}, {...definition, properties: {ActivatesTags: ['Customization.Fitting.Gloves']}}), /activated tags/);
...
assert.throws(() => m.assertResolved(item, definition, {parts: [{...part, skeletalMesh: m.SOURCE_MESH.replace(/_M\b/g, '_F')}]}, slots), /assault-gloves mesh/);
...
const dir = fs.mkdtempSync(path.join(os.tmpdir(), '.assault-gloves-freeze-'));
try {
...
const slots = [{slot: m.MATERIAL_SLOT, material: '/Game/Discovery/Characters/Military/Assets/AssaultGloves/M_Default.M_Default'}];
...
const definition = {formatVersion: 1, id: item.id, source: 'Discovery/Content/Gloves.uasset', sourceSha256: 'ab',
  properties: {Slots: ['Hands'], MaterialOverrides: [{Key: m.MATERIAL_SLOT, Value: soft(material)}], VisualParts: [part]}};
...
// Medium resolves one visible part on the gloves mesh with the cohort material; Light/Heavy overrides stay outside.
const ok = resolve(definition);
assert.deepEqual(ok.effectiveSlots, [{slot: m.MATERIAL_SLOT, material}]);
assert.deepEqual([ok.visible.length, ok.visible[0].skeletalMesh, ok.outfit.fittingTags], [1, m.SOURCE_MESH, []]);
resolve(variant({ActivatesTags: []}));
...
rejects(variant({ActivatesTags: ['Customization.Fitting.Gloves']}), /activated tags/);
rejects(variant({}, medium({bOverrideMesh: true, ReplacementSkeletalMesh: soft(LIGHT)})), /assault-gloves mesh/);
...
rejects(variant({MaterialOverrides: [{Key: 'AssaultPants', Value: soft(material)}]}), /absent slot/);
...
class ExposedSkinContracts(unittest.TestCase):
    def test_only_this_gloves_mask_is_removed_from_asset_and_resolved_parts(self):
        adapter = load_adapter()
        mask = {'bodyMaskUrl': 'mask.png', 'bodyMaskUvTiles': [2, 1], 'coverageSource': 'derived-projection'}
        other = {'sha256': 'other', **mask}
        document = {'meshes': {adapter.SOURCE_MESH: {'sha256': adapter.GLB_SHA256, 'url': 'gloves.glb', **mask},
                               '/Game/Other.Other': dict(other)},
                    'ready': [{'parts': [{'sourceMesh': adapter.SOURCE_MESH, 'url': 'gloves.glb', **mask},
                                          {'sourceMesh': '/Game/Other.Other', **mask}]}]}
        adapter._preserve_exposed_skin(document)
        self.assertEqual(document['meshes'][adapter.SOURCE_MESH], {'sha256': adapter.GLB_SHA256, 'url': 'gloves.glb'})
        self.assertEqual(document['ready'][0]['parts'][0], {'sourceMesh': adapter.SOURCE_MESH, 'url': 'gloves.glb'})
        self.assertEqual(document['meshes']['/Game/Other.Other'], other)
        self.assertEqual(document['ready'][0]['parts'][1], {'sourceMesh': '/Game/Other.Other', **mask})
        once = json.dumps(document)
        adapter._preserve_exposed_skin(document)
        self.assertEqual(json.dumps(document), once)

    def test_changed_mesh_cannot_inherit_the_reviewed_skin_policy(self):
        adapter = load_adapter()
        document = {'meshes': {adapter.SOURCE_MESH: {'sha256': 'different', 'bodyMaskUrl': 'mask.png'}}}
        with self.assertRaisesRegex(ValueError, 'Unreviewed glove geometry'):
            adapter._preserve_exposed_skin(document)
        self.assertEqual(document['meshes'][adapter.SOURCE_MESH]['bodyMaskUrl'], 'mask.png')
...
class FreezeContracts(unittest.TestCase):
    ...
    def test_adapter_and_freeze_share_the_handed_over_contract(self):
        ...
        names = ('SOURCE_MESH', 'MATERIAL_SLOT', 'ITEM_SLOT', 'CONTEXT', 'EXPECTED_COUNT', 'GLB_SHA256', 'MORPH_NAMES',
                 'MESH_FACTS', 'BASELINE_ADVERTISED', 'BASELINE_INDEXED', 'CATALOG_ITEMS')
        ...
        # The source-index count, not the product UI's touched total of 503.
        self.assertEqual((adapter.BASELINE_ADVERTISED, adapter.BASELINE_STRUCTURAL, adapter.BASELINE_INDEXED,
                          adapter.CATALOG_ITEMS), (456, 457, 457, 2866))
    ...
    def test_production_resolver_on_medium_admits_one_gloves_part(self):
