"""Stage the 11-choice Military Combat Gloves family (Medium) with the Large Sneakers orchestration.
...
skinning and named morph targets, the frozen cohort with its two full-glove fitting tags, active baseline and
build pairing around every stage. The combat-gloves mesh was already exported and converted into this batch
...
earlier folders are only read. Run from the repository root with C:/ProgramData/anaconda3/python.exe:
  ...
  python scripts/shader-probe/prepare-combat-gloves.py mesh      # validate/reuse the exact mesh, GLB, attributes, morphs
  node --import tsx _docs/combat-gloves-2026-09-15/freeze.mjs    # definitions/slots/tags/baseline -> resolved-cohort.json
  python scripts/shader-probe/prepare-combat-gloves.py source    # exact materials, parent chains, validate, textures
  python scripts/shader-probe/prepare-combat-gloves.py build     # compile, independent CPU check, surface/UV audit
  python scripts/shader-probe/prepare-combat-gloves.py gpu       # fresh uniquely named check-webgl run
  python scripts/shader-probe/prepare-combat-gloves.py index     # runtime staging, additive preview (no body mask)
  python scripts/shader-probe/prepare-combat-gloves.py coverage  # derived A/idle body coverage, then index again
"""
import argparse
...
DOCS = Path('_docs/combat-gloves-2026-09-15')
WORK = Path('scripts/generated/shader-probe/combat-gloves-v1')
RUNTIME = Path('public/models/reconstructed-combat-gloves-v1')
PREVIEW = Path('public/models/reconstructed-combat-gloves-preview-v1')
MESH_REPORT = DOCS / 'mesh-report.json'
SOURCE_MESH = '/Game/Discovery/Characters/Military/Assets/CombatGloves/SK_Military_CombatGloves_M.SK_Military_CombatGloves_M'
MATERIAL_SLOT = 'CombatGloves'
ITEM_SLOT = 'hands'
MARKER = 'shader-probe/prepare-combat-gloves'
EXPECTED_COUNT = 11
...
GLB_SHA256 = 'fb1ec530587e17a0a571e5deb5eed2a30074857f66db2e539e3bc2ce4fa32c84'
# The exported Medium mesh as handed over: original morph targets in source order, and the report summary.
# The two shrink morphs are sleeve fitting; production outfit rules activate them, never this adapter.
MORPH_NAMES = ['medium_male', 'medium_female', 'shrink_gloves_under_jacket', 'shrink_gloves_under_jacket_right']
MESH_FACTS = {'vertices': 5676, 'triangles': 9592, 'uvSets': 2, 'bones': 53, 'materialSections': 1}
# Every full-glove definition activates exactly these tags (source order is not significant).
FITTING_TAGS = ['Customization.HideMesh.NailsCovered', 'Customization.Shape.PushInsideClothes.push_gloves']
...
BASELINE_ADVERTISED, BASELINE_STRUCTURAL = 470, 471
BASELINE_INDEXED, CATALOG_ITEMS = 471, 2866
...
def _require_cohort():
    ...
    if cohort.get('meshes') != [SOURCE_MESH] or cohort.get('attached'):
        raise SystemExit('The cohort must name exactly the one unattached combat-gloves skeletal mesh')
...
def _require_mesh_request():
    ...
    if not manifest.is_file() or d.read_json(manifest) != [d.package_of(SOURCE_MESH)]:
        raise SystemExit('meshes-01 was not extracted for exactly the combat-gloves mesh request; use a new folder')
...
def _require_glb_geometry(report, dto):
    ...
    # Default weights (mesh or node) would apply the sleeve-fitting morphs to every outfit.
...
def _require_mesh_contract():
    """Hash-current preserved report, exact skeletal source identity/package, one CombatGloves slot, bones, UVs, morphs."""
    ...
    if report['sha256'] != GLB_SHA256:
        raise SystemExit('The mesh report GLB hash is not the preserved combat-gloves conversion; do not reconvert')
...
def _require_frozen_cohort(report, slots):
    ...
    for row, item in zip(frozen['items'], cohort['items']):
        ...
        if len(parts) != 1 or parts[0].get('mesh') != SOURCE_MESH or parts[0].get('slots') != [
            ...
            raise SystemExit(f'Frozen choice is not one {MATERIAL_SLOT} part on the combat-gloves mesh: {item["id"]}')
