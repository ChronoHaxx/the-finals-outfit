"""Stage the 11-choice Starter Set T-Shirt family (Medium) with the Large Sneakers orchestration.
...
skinning and named morph targets, the frozen cohort with its four torso fitting tags, active baseline and
build pairing around every stage. The T-shirt mesh was already exported and converted into this batch
...
earlier folders are only read. Run from the repository root with C:/ProgramData/anaconda3/python.exe:
  ...
  python scripts/shader-probe/prepare-starter-tshirts.py mesh      # validate/reuse the exact mesh, GLB, attributes, morphs
  node --import tsx _docs/starter-tshirts-2026-09-15/freeze.mjs    # definitions/slots/tags/baseline -> resolved-cohort.json
  python scripts/shader-probe/prepare-starter-tshirts.py source    # exact materials, parent chains, validate, textures
  python scripts/shader-probe/prepare-starter-tshirts.py build     # compile, independent CPU check, surface/UV audit
  python scripts/shader-probe/prepare-starter-tshirts.py gpu       # fresh uniquely named check-webgl run
  python scripts/shader-probe/prepare-starter-tshirts.py index     # runtime staging, additive preview (no body mask)
  python scripts/shader-probe/prepare-starter-tshirts.py coverage  # derived A/idle body coverage, then index again
"""
import argparse
...
DOCS = Path('_docs/starter-tshirts-2026-09-15')
WORK = Path('scripts/generated/shader-probe/starter-tshirts-v1')
RUNTIME = Path('public/models/reconstructed-starter-tshirts-v1')
PREVIEW = Path('public/models/reconstructed-starter-tshirts-preview-v1')
MESH_REPORT = DOCS / 'mesh-report.json'
SOURCE_MESH = '/Game/Discovery/Characters/StarterSet/Assets/TShirt/SK_StarterSet_TShirt_M.SK_StarterSet_TShirt_M'
MATERIAL_SLOT = 'TShirt'
ITEM_SLOT = 'upperBody'
MARKER = 'shader-probe/prepare-starter-tshirts'
EXPECTED_COUNT = 11
...
GLB_SHA256 = '6aa7672320be90b1692a0e61cb4e75d82c68b5c18139a38ec0f81c7cd4ee5d96'
# The exported Medium mesh as handed over: original morph targets in source order, and the report summary.
# Only the two Medium body morphs; this family has no sleeve morphs. The viewer drives them, never this adapter.
MORPH_NAMES = ['medium_male', 'medium_female']
MESH_FACTS = {'vertices': 1761, 'triangles': 2778, 'uvSets': 2, 'bones': 27, 'materialSections': 1}
# Every T-shirt definition activates exactly these tags (source order is not significant); no glove/nail tags.
FITTING_TAGS = ['Customization.Shape.PushInsideClothes.push_torso', 'Customization.HideMesh.WearingLongJacket',
...
# or a skin pair, as the source-index loader counts them (not the picker's full touched total of 528,
# which also counts legacy, review and body-paint states).
BASELINE_ADVERTISED, BASELINE_STRUCTURAL = 481, 482
BASELINE_INDEXED, CATALOG_ITEMS = 482, 2866
...
def _is_fitting_tags(tags):
    """Exactly the four T-shirt fitting tags in any order; missing, empty, repeated or other tags are drift."""
...
def _require_cohort():
    ...
    if cohort.get('meshes') != [SOURCE_MESH] or cohort.get('attached'):
        raise SystemExit('The cohort must name exactly the one unattached starter-tshirt skeletal mesh')
...
def _require_mesh_request():
    ...
    if not manifest.is_file() or d.read_json(manifest) != [d.package_of(SOURCE_MESH)]:
        raise SystemExit('meshes-01 was not extracted for exactly the starter-tshirt mesh request; use a new folder')
...
def _require_mesh_contract():
    """Hash-current preserved report, exact skeletal source identity/package, one TShirt slot, bones, UVs, morphs."""
    ...
    if report['sha256'] != GLB_SHA256:
        raise SystemExit('The mesh report GLB hash is not the preserved starter-tshirt conversion; do not reconvert')
...
def _require_frozen_cohort(report, slots):
    ...
    for row, item in zip(frozen['items'], cohort['items']):
        ...
        if len(parts) != 1 or parts[0].get('mesh') != SOURCE_MESH or parts[0].get('slots') != [
            ...
            raise SystemExit(f'Frozen choice is not one {MATERIAL_SLOT} part on the starter-tshirt mesh: {item["id"]}')
        # Both the source definition and the production resolver result carry exactly the four T-shirt tags.
