"""Stage the 10-choice Military Sniper Boots family (Medium) with the Large Sneakers orchestration.
...
earlier folders are only read. Run from the repository root with C:/ProgramData/anaconda3/python.exe:
  ...
  python scripts/shader-probe/prepare-sniper-boots.py mesh      # validate/reuse the exact mesh, GLB, attributes, morphs
  node --import tsx _docs/sniper-boots-2026-09-16/freeze.mjs    # definitions/slots/tags/baseline -> resolved-cohort.json
  python scripts/shader-probe/prepare-sniper-boots.py source    # exact materials, parent chains, validate, textures
  python scripts/shader-probe/prepare-sniper-boots.py build     # compile, independent CPU check, surface/UV audit
  python scripts/shader-probe/prepare-sniper-boots.py gpu       # fresh uniquely named check-webgl run
  python scripts/shader-probe/prepare-sniper-boots.py index     # runtime staging, additive preview (no body mask)
  python scripts/shader-probe/prepare-sniper-boots.py coverage  # derived A/idle body coverage, then index again
"""
import argparse
...
DOCS = Path('_docs/sniper-boots-2026-09-16')
WORK = Path('scripts/generated/shader-probe/sniper-boots-v1')
RUNTIME = Path('public/models/reconstructed-sniper-boots-v1')
PREVIEW = Path('public/models/reconstructed-sniper-boots-preview-v1')
MESH_REPORT = DOCS / 'mesh-report.json'
SOURCE_MESH = '/Game/Discovery/Characters/Military/Assets/SniperBoots/SK_Military_SniperBoots_M.SK_Military_SniperBoots_M'
MATERIAL_SLOT = 'SniperBoots'
ITEM_SLOT = 'feet'
MARKER = 'shader-probe/prepare-sniper-boots'
EXPECTED_COUNT = 10
...
GLB_SHA256 = '69f401a4311275046e5ddd3ed25e187ad0a912fc983f4a2fb2a3c40f3ada47b3'
# The exported Medium mesh as handed over: original morph targets in source order, and the report summary.
# Only the two Medium body morphs, in this mesh's own order (the reverse of the assault-vest mesh); a
# reordered pair is a different export. The viewer drives them, never this adapter.
MORPH_NAMES = ['medium_male', 'medium_female']
MESH_FACTS = {'vertices': 3080, 'triangles': 5028, 'uvSets': 2, 'bones': 12, 'materialSections': 1}
# Every boot activates exactly these two tags: trousers shrink into the low boots and the lower foot is pushed
# inside. Source order is not significant; missing, empty, repeated or extra tags are drift. These rules drive
# the source fitting of trousers and feet, so none of them is invented or dropped here.
...
EXPECTED_ACTIVATED_TAGS_BY_ITEM = {item: list(FITTING_TAGS) for item in (
    'military-sniperboots-leather',
    'military-sniperboots-leather-alfaacta',
    'military-sniperboots-leather-arctic',
    'military-sniperboots-leather-black',
    'military-sniperboots-leather-blackrabbit',
    'military-sniperboots-leather-dede',
    'military-sniperboots-leather-esportkz',
    'military-sniperboots-leather-fivefears',
    'military-sniperboots-leather-gray',
    'military-sniperboots-leather-white')}
...
# or a skin pair, as the source-index loader counts them (not the picker's full touched total of 549,
# which also counts legacy, review and body-paint states).
BASELINE_ADVERTISED, BASELINE_STRUCTURAL = 502, 503
BASELINE_INDEXED, CATALOG_ITEMS = 503, 2866
...
def _require_cohort():
    ...
    if cohort.get('meshes') != [SOURCE_MESH] or cohort.get('attached'):
        raise SystemExit('The cohort must name exactly the one unattached sniper-boot skeletal mesh')
...
def _require_mesh_request():
    ...
    if not manifest.is_file() or d.read_json(manifest) != [d.package_of(SOURCE_MESH)]:
        raise SystemExit('meshes-01 was not extracted for exactly the sniper-boot mesh request; use a new folder')
...
def _require_mesh_contract():
    """Hash-current preserved report, exact skeletal source identity/package, one SniperBoots slot, bones, UVs, morphs."""
    ...
    if report['sha256'] != GLB_SHA256:
        raise SystemExit('The mesh report GLB hash is not the preserved sniper-boot conversion; do not reconvert')
...
def _require_frozen_cohort(report, slots):
    ...
    for row, item in zip(frozen['items'], cohort['items']):
        ...
        if len(parts) != 1 or parts[0].get('mesh') != SOURCE_MESH or parts[0].get('slots') != [
            ...
            raise SystemExit(f'Frozen choice is not one {MATERIAL_SLOT} part on the sniper-boot mesh: {item["id"]}')
