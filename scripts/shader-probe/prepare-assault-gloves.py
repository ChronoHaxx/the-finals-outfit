"""Stage the 14-choice Military Assault Gloves family (Medium) with the Large Sneakers orchestration.
...
skinning and named morph targets, the frozen cohort, active baseline and build pairing around every stage.
The assault-gloves mesh was already exported and converted into this batch before handoff, so the mesh stage
only validates and reuses that exact conversion (pinned GLB hash) and never converts again. The active index,
catalogs and earlier folders are only read. Run from the repository root with C:/ProgramData/anaconda3/python.exe:

  python scripts/shader-probe/prepare-assault-gloves.py mesh      # validate/reuse the exact mesh, GLB, attributes, morphs
  node --import tsx _docs/assault-gloves-2026-09-15/freeze.mjs    # definitions/slots/baseline -> resolved-cohort.json
  python scripts/shader-probe/prepare-assault-gloves.py source    # exact materials, parent chains, validate, textures
  python scripts/shader-probe/prepare-assault-gloves.py build     # compile, independent CPU check, surface/UV audit
  python scripts/shader-probe/prepare-assault-gloves.py gpu       # fresh uniquely named check-webgl run
  python scripts/shader-probe/prepare-assault-gloves.py index     # runtime staging, additive preview (no body mask)
  python scripts/shader-probe/prepare-assault-gloves.py coverage  # derived A/idle body coverage, then index again
"""
import argparse
...
DOCS = Path('_docs/assault-gloves-2026-09-15')
WORK = Path('scripts/generated/shader-probe/assault-gloves-v1')
RUNTIME = Path('public/models/reconstructed-assault-gloves-v1')
PREVIEW = Path('public/models/reconstructed-assault-gloves-preview-v1')
MESH_REPORT = DOCS / 'mesh-report.json'
SOURCE_MESH = '/Game/Discovery/Characters/Military/Assets/AssaultGloves/SK_Military_AssaultGloves_M.SK_Military_AssaultGloves_M'
MATERIAL_SLOT = 'AssaultGloves'
ITEM_SLOT = 'hands'
MARKER = 'shader-probe/prepare-assault-gloves'
EXPECTED_COUNT = 14
...
GLB_SHA256 = '6d493ca9147d0c6ab538362ab1684b00f60f80e15ecaf0acb5db857996c384a6'
...
MESH_FACTS = {'vertices': 2804, 'triangles': 4880, 'uvSets': 2, 'bones': 46, 'materialSections': 1}
...
BASELINE_ADVERTISED, BASELINE_STRUCTURAL = 456, 457
BASELINE_INDEXED, CATALOG_ITEMS = 457, 2866
...
def _require_cohort():
    ...
    if cohort.get('meshes') != [SOURCE_MESH] or cohort.get('attached'):
        raise SystemExit('The cohort must name exactly the one unattached assault-gloves skeletal mesh')
...
def _require_mesh_request():
    ...
    if not manifest.is_file() or d.read_json(manifest) != [d.package_of(SOURCE_MESH)]:
        raise SystemExit('meshes-01 was not extracted for exactly the assault-gloves mesh request; use a new folder')
...
def _require_mesh_contract():
    """Hash-current preserved report, exact skeletal source identity/package, one AssaultGloves slot, bones, UVs, morphs."""
    ...
    if report['sha256'] != GLB_SHA256:
        raise SystemExit('The mesh report GLB hash is not the preserved assault-gloves conversion; do not reconvert')
...
def _require_frozen_cohort(report, slots):
    """freeze.mjs evidence still describes this cohort, this mesh report, Medium and the unchanged definitions."""
    ...
    for row, item in zip(frozen['items'], cohort['items']):
        ...
        if len(parts) != 1 or parts[0].get('mesh') != SOURCE_MESH or parts[0].get('slots') != [
            ...
            raise SystemExit(f'Frozen choice is not one {MATERIAL_SLOT} part on the assault-gloves mesh: {item["id"]}')
        # This family activates no fitting tags; a missing ActivatesTags is the same as an empty list.
        properties = (row.get('definition') or {}).get('properties') or {}
        if properties.get('ActivatesTags', []) != [] or row.get('fittingTags') != []:
            raise SystemExit(f'Frozen choice activates tags; this family has none: {item["id"]}')
...
def _preserve_exposed_skin(document):
    """This fingerless Medium mesh fits over skin; its projected mask cuts exposed fingers.

    Keep the derived mask as diagnostic evidence, but remove only this exact mesh's
    mask bindings. Run on both the asset map and resolved parts so rebuilding the
    preview cannot reintroduce it. Other garments retain their own coverage.
    """
    if isinstance(document, dict):
        entry = document.get('meshes', {}).get(SOURCE_MESH) if isinstance(document.get('meshes'), dict) else None
        if entry is not None:
            if entry.get('sha256') != GLB_SHA256:
                raise ValueError('Unreviewed glove geometry: exposed-skin policy requires the preserved Medium mesh')
            for key in ('bodyMaskUrl', 'bodyMaskUvTiles', 'coverageSource'):
                entry.pop(key, None)
        if document.get('sourceMesh') == SOURCE_MESH:
            for key in ('bodyMaskUrl', 'bodyMaskUvTiles', 'coverageSource'):
                document.pop(key, None)
        for value in document.values():
            _preserve_exposed_skin(value)
...
def _apply_coverage_policy():
    for file in (RUNTIME / 'assets-additions.json', PREVIEW / 'assets.json',
                 PREVIEW / 'supported-items.json', WORK / 'resolver' / 'after.json'):
        document = d.read_json(file)
        _preserve_exposed_skin(document)
        d.write(file, document)
    preview = d.read_json(PREVIEW / 'preview.json')
    preview['coveragePolicy'] = {
        'mode': 'preserve-body-skin', 'sourceMesh': SOURCE_MESH, 'meshSha256': GLB_SHA256,
        'scope': 'Medium; zero fitting weights; A and idle poses',
        'reason': 'Derived projection hides exposed fingers. Preserve body skin under the original fingerless glove.',
        'derivedMaskRetainedForDiagnosis': True,
    }
    d.write(PREVIEW / 'preview.json', preview)
