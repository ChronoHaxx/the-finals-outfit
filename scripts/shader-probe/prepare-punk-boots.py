"""Stage the 10-choice Streetwear Punk Boots family (Medium) with the Large Sneakers orchestration.
...
build pairing around every stage. The boot mesh was already exported and converted into this batch
...
earlier folders are only read. Run from the repository root with C:/ProgramData/anaconda3/python.exe:
  ...
  python scripts/shader-probe/prepare-punk-boots.py mesh      # validate/reuse the exact mesh, GLB, attributes, morphs
  node --import tsx _docs/punk-boots-2026-09-16/freeze.mjs    # definitions/slots/tags/baseline -> resolved-cohort.json
  python scripts/shader-probe/prepare-punk-boots.py source    # exact materials, parent chains, validate, textures
  python scripts/shader-probe/prepare-punk-boots.py build     # compile, independent CPU check, surface/UV audit
  python scripts/shader-probe/prepare-punk-boots.py gpu       # fresh uniquely named check-webgl run
  python scripts/shader-probe/prepare-punk-boots.py index     # runtime staging, additive preview (no body mask)
  python scripts/shader-probe/prepare-punk-boots.py coverage  # derived A/idle body coverage, then index again
"""
import argparse
...
DOCS = Path('_docs/punk-boots-2026-09-16')
WORK = Path('scripts/generated/shader-probe/punk-boots-v1')
RUNTIME = Path('public/models/reconstructed-punk-boots-v1')
PREVIEW = Path('public/models/reconstructed-punk-boots-preview-v1')
MESH_REPORT = DOCS / 'mesh-report.json'
SOURCE_MESH = '/Game/Discovery/Characters/Streetwear/Assets/PunkBoots/SK_Streetwear_PunkBoots_M.SK_Streetwear_PunkBoots_M'
MATERIAL_SLOT = 'PunkBoots'
ITEM_SLOT = 'feet'
MARKER = 'shader-probe/prepare-punk-boots'
EXPECTED_COUNT = 10
...
GLB_SHA256 = '122ca4eaaefc527b56e608e4939af6f9a66ec29705f876b1945f8532bd460537'
# The exported Medium mesh as handed over: original morph targets in source order, and the report summary.
# Only the two Medium body morphs, in this mesh's own order (the reverse of the sniper-boot mesh); a
# reordered pair is a different export. The viewer drives them, never this adapter.
MORPH_NAMES = ['medium_female', 'medium_male']
MESH_FACTS = {'vertices': 4638, 'triangles': 7442, 'uvSets': 2, 'bones': 16, 'materialSections': 1}
# Every boot activates exactly these three tags: the lower foot is pushed inside, and trousers shrink under both
# the low-boot and the wide-boot rules. Source order is not significant; missing, empty, repeated or extra tags
# are drift. These rules drive the source fitting of trousers and feet, so none of them is invented or dropped here.
FITTING_TAGS = ['Customization.Shape.PushInsideClothes.push_lower_feet',
...
EXPECTED_ACTIVATED_TAGS_BY_ITEM = {item: list(FITTING_TAGS) for item in (
    'streetwear-punkboots-canvas',
    'streetwear-punkboots-leather',
    'streetwear-punkboots-leather-blackwhite',
    'streetwear-punkboots-leather-brown',
    'streetwear-punkboots-leather-lightbrown',
    'streetwear-punkboots-leather-orf',
    'streetwear-punkboots-leather-shiny',
    'streetwear-punkboots-wornleather',
    'streetwear-punkboots-wornleather-biker',
    'streetwear-punkboots-wornleather-black')}
...
# or a skin pair, as the source-index loader counts them (not the picker's full touched total of 569,
# which also counts legacy, review and body-paint states).
BASELINE_ADVERTISED, BASELINE_STRUCTURAL = 522, 523
BASELINE_INDEXED, CATALOG_ITEMS = 523, 2866
...
def _require_cohort():
    ...
    if cohort.get('meshes') != [SOURCE_MESH] or cohort.get('attached'):
        raise SystemExit('The cohort must name exactly the one unattached punk-boot skeletal mesh')
...
def _require_mesh_request():
    ...
    if not manifest.is_file() or d.read_json(manifest) != [d.package_of(SOURCE_MESH)]:
        raise SystemExit('meshes-01 was not extracted for exactly the punk-boot mesh request; use a new folder')
...
def _require_mesh_contract():
    """Hash-current preserved report, exact skeletal source identity/package, one PunkBoots slot, bones, UVs, morphs."""
    ...
    if report['sha256'] != GLB_SHA256:
        raise SystemExit('The mesh report GLB hash is not the preserved punk-boot conversion; do not reconvert')
...
def _require_frozen_cohort(report, slots):
    ...
    for row, item in zip(frozen['items'], cohort['items']):
        ...
        if len(parts) != 1 or parts[0].get('mesh') != SOURCE_MESH or parts[0].get('slots') != [
            ...
            raise SystemExit(f'Frozen choice is not one {MATERIAL_SLOT} part on the punk-boot mesh: {item["id"]}')
