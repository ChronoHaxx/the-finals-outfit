"""Stage the 20-choice Streetwear Wristbands family (Medium) with the Large Sneakers orchestration.
...
preview, derived A/idle coverage); this file only re-points its paths at this batch. The wristband
...
repository root with C:/ProgramData/anaconda3/python.exe:
  ...
  python scripts/shader-probe/prepare-wristbands.py mesh      # fresh exact mesh export, GLB, attribute check
  node --import tsx _docs/wristbands-2026-09-13/freeze.mjs    # definitions/slots -> resolved-cohort.json
  python scripts/shader-probe/prepare-wristbands.py source    # exact materials, parent chains, validate, textures
  python scripts/shader-probe/prepare-wristbands.py build     # compile, independent CPU check, surface audit
  python scripts/shader-probe/prepare-wristbands.py gpu       # fresh uniquely named check-webgl run
  python scripts/shader-probe/prepare-wristbands.py index     # runtime staging, additive preview (no body mask)
  python scripts/shader-probe/prepare-wristbands.py coverage  # derived A/idle body coverage, then index again
"""
import argparse
...
DOCS = Path('_docs/wristbands-2026-09-13')
WORK = Path('scripts/generated/shader-probe/wristbands-v1')
RUNTIME = Path('public/models/reconstructed-wristbands-v1')
PREVIEW = Path('public/models/reconstructed-wristbands-preview-v1')
MESH_REPORT = DOCS / 'mesh-report.json'
SOURCE_MESH = ('/Game/Discovery/Characters/Streetwear/Assets/Wristband/SK_Streetwear_Wristband_M.'
               'SK_Streetwear_Wristband_M')
MATERIAL_SLOT = 'Wristband'
MARKER = 'shader-probe/prepare-wristbands'
...
def mesh():
    ...
    if cohort.get('meshes') != [SOURCE_MESH]:
        raise SystemExit('The frozen cohort must name exactly the one wristband skeletal mesh')
    ...
    if not _is_source_mesh(record.get('path')):
        raise SystemExit('The exported record is not the requested wristband skeletal mesh')
