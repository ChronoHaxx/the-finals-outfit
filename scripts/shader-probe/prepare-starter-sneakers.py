"""Stage the 17-choice Starter Set Sneakers family (Medium) with the Large Sneakers orchestration.
...
this file only re-points its paths at this batch and rechecks the source mesh, frozen cohort and build
pairing around every stage. The starter-sneaker mesh was already exported and converted into this batch
before handoff, so the mesh stage only validates and reuses that exact conversion (pinned GLB hash) and never
converts again. The active index, catalogs and earlier folders are only read. Run from the repository root
with C:/ProgramData/anaconda3/python.exe:

  python scripts/shader-probe/prepare-starter-sneakers.py mesh      # validate/reuse the exact mesh, GLB, attributes
  node --import tsx _docs/starter-sneakers-2026-09-15/freeze.mjs    # definitions/slots -> resolved-cohort.json
  python scripts/shader-probe/prepare-starter-sneakers.py source    # exact materials, parent chains, validate, textures
  python scripts/shader-probe/prepare-starter-sneakers.py build     # compile, independent CPU check, surface/UV audit
  python scripts/shader-probe/prepare-starter-sneakers.py gpu       # fresh uniquely named check-webgl run
  python scripts/shader-probe/prepare-starter-sneakers.py index     # runtime staging, additive preview (no body mask)
  python scripts/shader-probe/prepare-starter-sneakers.py coverage  # derived A/idle body coverage, then index again
"""
import argparse
...
DOCS = Path('_docs/starter-sneakers-2026-09-15')
WORK = Path('scripts/generated/shader-probe/starter-sneakers-v1')
RUNTIME = Path('public/models/reconstructed-starter-sneakers-v1')
PREVIEW = Path('public/models/reconstructed-starter-sneakers-preview-v1')
MESH_REPORT = DOCS / 'mesh-report.json'
SOURCE_MESH = '/Game/Discovery/Characters/StarterSet/Assets/Sneakers/SK_Sneakers_M.SK_Sneakers_M'
MATERIAL_SLOT = 'Sneakers'
ITEM_SLOT = 'feet'
MARKER = 'shader-probe/prepare-starter-sneakers'
EXPECTED_COUNT = 17
...
GLB_SHA256 = 'b7cafddaf56c61c431ff07470297d6bae3303dc981b66f700c71dc367f2513c3'
# Active index before this batch. Its one structurally ready but unadvertised entry (the knight pants)
# must stay ready and unadvertised in the preview.
BASELINE_ADVERTISED, BASELINE_STRUCTURAL = 426, 427
...
def _require_cohort():
    ...
    if cohort.get('meshes') != [SOURCE_MESH]:
        raise SystemExit('The cohort must name exactly the one starter-sneaker skeletal mesh')
...
def _require_mesh_request():
    ...
    if not manifest.is_file() or d.read_json(manifest) != [d.package_of(SOURCE_MESH)]:
        raise SystemExit('meshes-01 was not extracted for exactly the starter-sneaker mesh request; use a new folder')
...
def _require_mesh_contract():
    """Hash-current preserved report, exact skeletal source identity/package, one Sneakers slot, bones and original UVs."""
    ...
    if report['sha256'] != GLB_SHA256:
        raise SystemExit('The mesh report GLB hash is not the preserved starter-sneaker conversion; do not reconvert')
...
def _require_frozen_cohort(report, slots):
    ...
    for row, item in zip(frozen['items'], cohort['items']):
        ...
        if len(parts) != 1 or parts[0].get('mesh') != SOURCE_MESH or parts[0].get('slots') != [
            ...
            raise SystemExit(f'Frozen choice is not one {MATERIAL_SLOT} part on the starter-sneaker mesh: {item["id"]}')
...
def mesh():
    ...
    print(f'Mesh: {SOURCE_MESH} -> {report["file"]} (slot {MATERIAL_SLOT}; validated preserved conversion)', flush=True)
