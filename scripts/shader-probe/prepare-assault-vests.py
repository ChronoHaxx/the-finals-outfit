"""Stage the 10-choice Military Assault Vest family (Medium) with the Large Sneakers orchestration.
...
skinning and named morph targets, the frozen cohort with its per-choice fitting tags, active baseline and
build pairing around every stage. The vest mesh was already exported and converted into this batch
before handoff, so the mesh stage only validates and reuses that exact conversion (pinned GLB hash) and never
converts again. Body coverage is the shared derived A/idle pipeline, unchanged. The active index, catalogs and
earlier folders are only read. Run from the repository root with C:/ProgramData/anaconda3/python.exe:

  python scripts/shader-probe/prepare-assault-vests.py mesh      # validate/reuse the exact mesh, GLB, attributes, morphs
  node --import tsx _docs/assault-vests-2026-09-15/freeze.mjs    # definitions/slots/tags/baseline -> resolved-cohort.json
  python scripts/shader-probe/prepare-assault-vests.py source    # exact materials, parent chains, validate, textures
  python scripts/shader-probe/prepare-assault-vests.py build     # compile, independent CPU check, surface/UV audit
  python scripts/shader-probe/prepare-assault-vests.py gpu       # fresh uniquely named check-webgl run
  python scripts/shader-probe/prepare-assault-vests.py index     # runtime staging, additive preview (no body mask)
  python scripts/shader-probe/prepare-assault-vests.py coverage  # derived A/idle body coverage, then index again
"""
import argparse
...
DOCS = Path('_docs/assault-vests-2026-09-15')
WORK = Path('scripts/generated/shader-probe/assault-vests-v1')
RUNTIME = Path('public/models/reconstructed-assault-vests-v1')
PREVIEW = Path('public/models/reconstructed-assault-vests-preview-v1')
MESH_REPORT = DOCS / 'mesh-report.json'
SOURCE_MESH = '/Game/Discovery/Characters/Military/Assets/AssaultVest/SK_Military_AssaultVest_M.SK_Military_AssaultVest_M'
MATERIAL_SLOT = 'AssaultVest'
ITEM_SLOT = 'upperBody'
MARKER = 'shader-probe/prepare-assault-vests'
EXPECTED_COUNT = 10
...
GLB_SHA256 = '2c0e14a13c5c3f1a20ac54b28f8f280afde446bfad0edc43a31931d199c1b0e2'
# The exported Medium mesh as handed over: original morph targets in source order, and the report summary.
# Only the two Medium body morphs, and in this family's own order (the reverse of the T-shirt mesh); a
# reordered pair is a different export. The viewer drives them, never this adapter.
MORPH_NAMES = ['medium_female', 'medium_male']
MESH_FACTS = {'vertices': 19020, 'triangles': 29023, 'uvSets': 2, 'bones': 52, 'materialSections': 1}
# Fitting tags vary per choice. Every vest activates these eight; source order is not significant, and
# missing, empty, repeated or extra tags are drift. These rules drive gloves, wrist visibility, skirts,
# collars, badges and back accessories, so none of them is invented or dropped here.
COMMON_TAGS = ['Customization.HideMesh.WearingLongJacket',
               'Customization.HideMesh.WristsCovered',
               'Customization.Shape.PushBadge',
               'Customization.Shape.PushInsideClothes.push_full_jacket',
               'Customization.Shape.PushUpperBackItem',
               'Customization.Shape.ShrinkWrap.shrink_gloves_under_jacket',
               'Customization.Shape.ShrinkWrap.shrink_half_skirt_in',
               'Customization.Shape.ShrinkWrap.shrink_skirt_shorts_in']
UNDER_COLLAR = 'Customization.Shape.ShrinkWrap.shrink_under_collar'
# One entry per source choice: whether that choice also shrinks under the collar. Four of the ten do not,
# so the tag is never added family-wide; each definition is validated against its own row.
SHRINKS_UNDER_COLLAR = {'military-assaultvest-polyester-black': True,
                        'military-assaultvest-polyester-blackyellowospuze': True,
                        'military-assaultvest-polyester-greencamo': False,
                        'military-assaultvest-polyester-greenorange': True,
                        'military-assaultvest-polyester-junglecamo': True,
                        'military-assaultvest-polyester-moolahcamo': False,
                        'military-assaultvest-polyester-red': True,
                        'military-assaultvest-polyester-redblue': False,
                        'military-assaultvest-polyester-snowcamo': True,
                        'military-assaultvest-polyester-swat': False}
EXPECTED_ACTIVATED_TAGS_BY_ITEM = {item: [*COMMON_TAGS, *([UNDER_COLLAR] if collar else [])]
                                   for item, collar in SHRINKS_UNDER_COLLAR.items()}
...
# or a skin pair, as the source-index loader counts them (not the picker's full touched total of 539,
# which also counts legacy, review and body-paint states).
BASELINE_ADVERTISED, BASELINE_STRUCTURAL = 492, 493
BASELINE_INDEXED, CATALOG_ITEMS = 493, 2866
...
def _is_fitting_tags(item_id, tags):
    """Exactly this choice's own tags in any order; missing, empty, repeated, other tags or an unknown id are drift."""
    expected = EXPECTED_ACTIVATED_TAGS_BY_ITEM.get(item_id)
    if expected is None or not isinstance(tags, list) or not all(isinstance(tag, str) for tag in tags):
        return False
...
def _require_cohort():
    ...
    if cohort.get('meshes') != [SOURCE_MESH] or cohort.get('attached'):
        raise SystemExit('The cohort must name exactly the one unattached assault-vest skeletal mesh')
    ...
    if sorted(ids) != sorted(EXPECTED_ACTIVATED_TAGS_BY_ITEM):
        raise SystemExit('The cohort choices differ from the per-choice fitting tag mapping')
...
def _require_mesh_request():
    ...
    if not manifest.is_file() or d.read_json(manifest) != [d.package_of(SOURCE_MESH)]:
        raise SystemExit('meshes-01 was not extracted for exactly the assault-vest mesh request; use a new folder')
...
def _require_glb_geometry(report, dto):
    ...
    # Default weights (mesh or node) would apply a Medium body morph to every outfit.
...
def _require_mesh_contract():
    """Hash-current preserved report, exact skeletal source identity/package, one AssaultVest slot, bones, UVs, morphs."""
    ...
    if report['sha256'] != GLB_SHA256:
        raise SystemExit('The mesh report GLB hash is not the preserved assault-vest conversion; do not reconvert')
...
def _require_frozen_cohort(report, slots):
    """freeze.mjs evidence still describes this cohort, this mesh report, Medium, the per-choice tags and the definitions."""
    ...
    for row, item in zip(frozen['items'], cohort['items']):
        ...
        if len(parts) != 1 or parts[0].get('mesh') != SOURCE_MESH or parts[0].get('slots') != [
            ...
            raise SystemExit(f'Frozen choice is not one {MATERIAL_SLOT} part on the assault-vest mesh: {item["id"]}')
        # Both the source definition and the production resolver result carry exactly this choice's own tags.
        properties = (row.get('definition') or {}).get('properties') or {}
        if not _is_fitting_tags(item['id'], properties.get('ActivatesTags')) \
                or not _is_fitting_tags(item['id'], row.get('fittingTags')):
            raise SystemExit(f'Frozen choice does not activate exactly '
                             f'{EXPECTED_ACTIVATED_TAGS_BY_ITEM.get(item["id"])}: {item["id"]}')
