"""Stage the 10-choice Streetwear Pants family (Medium) with the Large Sneakers orchestration.
...
this file only re-points its paths and hooks at this batch and rechecks the source mesh, its one original UV set,
skinning and named morph targets, the frozen cohort with its per-choice fitting tags, active baseline and
build pairing around every stage. The pants mesh was already exported and converted into this batch
...
earlier folders are only read. Run from the repository root with C:/ProgramData/anaconda3/python.exe:
  ...
  python scripts/shader-probe/prepare-streetwear-pants.py mesh      # validate/reuse the exact mesh, GLB, attributes, morphs
  node --import tsx _docs/streetwear-pants-2026-09-16/freeze.mjs    # definitions/slots/tags/baseline -> resolved-cohort.json
  python scripts/shader-probe/prepare-streetwear-pants.py source    # exact materials, parent chains, validate, textures
  python scripts/shader-probe/prepare-streetwear-pants.py build     # compile, independent CPU check, surface/UV audit
  python scripts/shader-probe/prepare-streetwear-pants.py gpu       # fresh uniquely named check-webgl run
  python scripts/shader-probe/prepare-streetwear-pants.py index     # runtime staging, additive preview (no body mask)
  python scripts/shader-probe/prepare-streetwear-pants.py coverage  # derived A/idle body coverage, then index again
"""
import argparse
...
DOCS = Path('_docs/streetwear-pants-2026-09-16')
WORK = Path('scripts/generated/shader-probe/streetwear-pants-v1')
RUNTIME = Path('public/models/reconstructed-streetwear-pants-v1')
PREVIEW = Path('public/models/reconstructed-streetwear-pants-preview-v1')
MESH_REPORT = DOCS / 'mesh-report.json'
SOURCE_MESH = '/Game/Discovery/Characters/Streetwear/Assets/Pants/SK_Streetwear_Pants_M.SK_Streetwear_Pants_M'
MATERIAL_SLOT = 'Pants'
ITEM_SLOT = 'lowerBody'
MARKER = 'shader-probe/prepare-streetwear-pants'
EXPECTED_COUNT = 10
...
GLB_SHA256 = '06769c4645f87fd5500f0aaafd26dc9136acef1a59e0950b55a2b4031fa93581'
# The exported Medium mesh as handed over: original morph targets in source order, and the report summary.
# Only the two Medium body morphs, in this mesh's own order; a reordered pair is a different export.
# The viewer drives them, never this adapter.
MORPH_NAMES = ['medium_male', 'medium_female']
# Unlike the sniper-boot mesh, this export has one original UV set (TEXCOORD_0); nothing adds a second.
MESH_FACTS = {'vertices': 2172, 'triangles': 3673, 'uvSets': 1, 'bones': 22, 'materialSections': 1}
# Every pair activates exactly these three tags: the right calf, thighs and upper thighs are pushed inside the
# clothing. They add no cuff morphs to this mesh. Source order is not significant; missing, empty, repeated or
# extra tags are drift. These rules drive the source fitting of the body, so none of them is invented or dropped here.
FITTING_TAGS = ['Customization.Shape.PushInsideClothes.push_right_calf',
                'Customization.Shape.PushInsideClothes.push_thighs',
                'Customization.Shape.PushInsideClothes.push_upper_thighs']
# One row per source choice; each definition is validated against its own row and an id without one is unknown.
EXPECTED_ACTIVATED_TAGS_BY_ITEM = {item: list(FITTING_TAGS) for item in (
    'streetwear-pants-cotton-black',
    'streetwear-pants-cotton-blackwhite',
    'streetwear-pants-cotton-blue',
    'streetwear-pants-cotton-camo',
    'streetwear-pants-cotton-kawaii',
    'streetwear-pants-cotton-pink',
    'streetwear-pants-cotton-sponsoriseult',
    'streetwear-pants-cotton-sponsoriseultblue',
    'streetwear-pants-cotton-whiteblack',
    'streetwear-pants-cotton-yellow')}
...
# or a skin pair, as the source-index loader counts them (not the picker's full touched total of 559,
# which also counts legacy, review and body-paint states).
BASELINE_ADVERTISED, BASELINE_STRUCTURAL = 512, 513
BASELINE_INDEXED, CATALOG_ITEMS = 513, 2866
...
def geometry_blocked():
    ...
    This mesh has only UV set 0, so a manifest without a declared requirement keeps the two-set default and
    stays blocked rather than being assumed to need one set; no UV set is fabricated to admit it.
    """
...
def _require_cohort():
    ...
    if cohort.get('meshes') != [SOURCE_MESH] or cohort.get('attached'):
        raise SystemExit('The cohort must name exactly the one unattached streetwear-pants skeletal mesh')
...
def _require_mesh_request():
    ...
    if not manifest.is_file() or d.read_json(manifest) != [d.package_of(SOURCE_MESH)]:
        raise SystemExit('meshes-01 was not extracted for exactly the streetwear-pants mesh request; use a new folder')
...
def _require_mesh_contract():
    """Hash-current preserved report, exact skeletal source identity/package, one Pants slot, bones, UVs, morphs."""
    ...
    if report['sha256'] != GLB_SHA256:
        raise SystemExit('The mesh report GLB hash is not the preserved streetwear-pants conversion; do not reconvert')
...
def _require_frozen_cohort(report, slots):
    ...
    for row, item in zip(frozen['items'], cohort['items']):
        ...
        if len(parts) != 1 or parts[0].get('mesh') != SOURCE_MESH or parts[0].get('slots') != [
            ...
            raise SystemExit(f'Frozen choice is not one {MATERIAL_SLOT} part on the streetwear-pants mesh: {item["id"]}')
