"""Stage the 15-choice Military Assault Pants family (Medium) with the Large Sneakers orchestration.
...
The assault-pants mesh was already exported and converted into this batch before handoff, so the mesh stage
...
catalogs and earlier folders are only read. Run from the repository root with C:/ProgramData/anaconda3/python.exe:
  ...
  python scripts/shader-probe/prepare-assault-pants.py mesh      # validate/reuse the exact mesh, GLB, attributes, morphs
  node --import tsx _docs/assault-pants-2026-09-15/freeze.mjs    # definitions/slots/baseline -> resolved-cohort.json
  python scripts/shader-probe/prepare-assault-pants.py source    # exact materials, parent chains, validate, textures
  python scripts/shader-probe/prepare-assault-pants.py build     # compile, independent CPU check, surface/UV audit
  python scripts/shader-probe/prepare-assault-pants.py gpu       # fresh uniquely named check-webgl run
  python scripts/shader-probe/prepare-assault-pants.py index     # runtime staging, additive preview (no body mask)
  python scripts/shader-probe/prepare-assault-pants.py coverage  # derived A/idle body coverage, then index again
"""
import argparse
...
DOCS = Path('_docs/assault-pants-2026-09-15')
WORK = Path('scripts/generated/shader-probe/assault-pants-v1')
RUNTIME = Path('public/models/reconstructed-assault-pants-v1')
PREVIEW = Path('public/models/reconstructed-assault-pants-preview-v1')
MESH_REPORT = DOCS / 'mesh-report.json'
SOURCE_MESH = '/Game/Discovery/Characters/Military/Assets/AssaultPants/SK_Military_AssaultPants_M.SK_Military_AssaultPants_M'
MATERIAL_SLOT = 'AssaultPants'
ITEM_SLOT = 'lowerBody'
MARKER = 'shader-probe/prepare-assault-pants'
EXPECTED_COUNT = 15
...
GLB_SHA256 = '99a2c56a4cbbf9628a43083a4c60595a1fddfb4c06cf11c5f01eb9b15e3dd9b6'
# Active index before this batch. Its one structurally ready but unadvertised entry (the knight pants)
# must stay ready and unadvertised in the preview. Touched: catalog choices the picker counts as
# reconstructed (advertised assemblies plus skin pairs) out of every catalog choice.
BASELINE_ADVERTISED, BASELINE_STRUCTURAL = 442, 443
BASELINE_INDEXED, CATALOG_ITEMS = 443, 2866
...
def _require_cohort():
    ...
    if cohort.get('meshes') != [SOURCE_MESH]:
        raise SystemExit('The cohort must name exactly the one assault-pants skeletal mesh')
...
def _require_mesh_request():
    ...
    if not manifest.is_file() or d.read_json(manifest) != [d.package_of(SOURCE_MESH)]:
        raise SystemExit('meshes-01 was not extracted for exactly the assault-pants mesh request; use a new folder')
...
def _dto_morph_names(dto):
    """Morph target names of the exported LOD0, in source order."""
...
def _require_mesh_contract():
    """Hash-current preserved report, exact skeletal source identity/package, one AssaultPants slot, bones, UVs, morphs."""
    ...
    if report['sha256'] != GLB_SHA256:
        raise SystemExit('The mesh report GLB hash is not the preserved assault-pants conversion; do not reconvert')
...
def _require_frozen_cohort(report, slots):
    ...
    for row, item in zip(frozen['items'], cohort['items']):
        ...
        if len(parts) != 1 or parts[0].get('mesh') != SOURCE_MESH or parts[0].get('slots') != [
            ...
            raise SystemExit(f'Frozen choice is not one {MATERIAL_SLOT} part on the assault-pants mesh: {item["id"]}')
